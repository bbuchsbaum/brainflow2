...
use bridge_types::{VolumeHandleInfo, VolumeType, TimeSeriesInfo, Loader};
use uuid::Uuid;
...
impl TemplateService {
    ...
    pub fn new(cache_dir: std::path::PathBuf) -> Result<Self, TemplateError> {
        ...
        let http_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| TemplateError::LoadFailed(format!("Failed to create HTTP client: {}", e)))?;
    ...
    fn sanitize_cache_dir(cache_dir: std::path::PathBuf) -> Result<std::path::PathBuf, TemplateError> {
        ...
        let abs_path = cache_dir.canonicalize()
            .map_err(|e| TemplateError::PathSecurityViolation(
    ...
    pub async fn get_filtered_templates(&self, filter: &TemplateFilter) -> Result<Vec<TemplateCatalogEntry>, TemplateError> {
        let catalog = self.catalog.read().await;
        Ok(catalog.get_filtered(filter))
    }
    
    /// Get a specific template entry by ID
    pub async fn get_template_entry(&self, template_id: &str) -> Result<Option<TemplateCatalogEntry>, TemplateError> {
        let catalog = self.catalog.read().await;
        Ok(catalog.get_by_id(template_id).cloned())
    }
    
    /// Get templates organized for menu building
    pub async fn get_menu_structure(&self) -> Result<HashMap<TemplateSpace, HashMap<TemplateType, Vec<TemplateCatalogEntry>>>, TemplateError> {
        let catalog = self.catalog.read().await;
    ...
    pub async fn load_template(&self, config: TemplateConfig) -> Result<TemplateLoadResult, TemplateError> {
        ...
        self.emit_progress(&template_id, LoadingStage::CheckingCache, 0.0, "Checking cache...").await;
        
        // Get template entry from catalog
        let entry = {
            let catalog = self.catalog.read().await;
            catalog.get_by_id(&template_id)
        ...
        // Emit loading stage
        self.emit_progress(&template_id, LoadingStage::Loading, 0.8, "Loading template data...").await;
        ...
        // Emit completion
        self.emit_progress(&template_id, LoadingStage::Complete, 1.0, "Template loaded successfully").await;
    ...
    async fn download_template(&self, entry: &TemplateCatalogEntry, cache_path: &Path) -> Result<(), TemplateError> {
        let download_url = entry.download_url.as_ref()
            .ok_or_else(|| TemplateError::DownloadFailed("No download URL available".to_string()))?;
        ...
        if let Some(parent) = cache_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| TemplateError::IoError(format!("Failed to create cache directory: {}", e)))?;
        }
        
        // Emit download progress
        self.emit_progress(&entry.id, LoadingStage::Downloading, 0.1, "Starting download...").await;
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk
                .map_err(|e| TemplateError::DownloadFailed(format!("Download stream error: {}", e)))?;
            ...
            if total_size > 0 {
                ...
                self.emit_progress(
                    ...
                    progress, 
                    &format!("Downloaded {:.1} MB of {:.1} MB", 
        ...
        fs::rename(&temp_path, cache_path)
            .map_err(|e| TemplateError::IoError(format!("Failed to move temp file to cache: {}", e)))?;
    ...
    async fn load_template_volume(&self, template_id: &str, cache_path: &Path) -> Result<VolumeHandleInfo, TemplateError> {
        ...
        if !cache_path.exists() {
            return Err(TemplateError::IoError(
                format!("Template file not found: {}", cache_path.display())
            ));
        }

        if !brainflow_loaders::is_loadable(cache_path) {
            return Err(TemplateError::IoError(
                format!("Template file format not supported: {}", cache_path.display())
            ));
        }
        ...
        let (volume_sendable, _affine) = nifti_loader::load_nifti_volume_auto(cache_path)
            .map_err(|e| TemplateError::IoError(
                format!("Failed to load template volume {}: {}", cache_path.display(), e)
            ))?;

        // Extract metadata from the loaded volume
        let loaded_data = nifti_loader::NiftiLoader::load(cache_path)
            .map_err(|e| TemplateError::IoError(
                format!("Failed to load template metadata for {}: {}", cache_path.display(), e)
            ))?;
        ...
        let (volume_type, time_series_info) = match &volume_sendable {
            bridge_types::VolumeSendable::VolF32(vol, _) => {
                ...
                if vol_dims.len() > 3 && vol_dims[3] > 1 {
                    (VolumeType::TimeSeries4D, Some(TimeSeriesInfo {
    ...
    async fn emit_progress(&self, template_id: &str, stage: LoadingStage, progress: f32, message: &str) {
    ...
    pub async fn get_cache_stats(&self) -> Result<TemplateCacheStats, TemplateError> {
        ...
        if self.cache_dir.exists() {
            let mut entries = fs::read_dir(&self.cache_dir)
                .map_err(|e| TemplateError::IoError(format!("Failed to read cache directory: {}", e)))?;
            ...
            while let Some(entry) = entries.next() {
                    ...
                    .is_file() 
                {
                    let metadata = entry.metadata()
                        .map_err(|e| TemplateError::IoError(format!("Failed to get file metadata: {}", e)))?;
    ...
    pub async fn clear_cache(&self) -> Result<(), TemplateError> {
        if self.cache_dir.exists() {
            ...
            fs::create_dir_all(&self.cache_dir)
                .map_err(|e| TemplateError::CacheError(format!("Failed to recreate cache directory: {}", e)))?;
        }
