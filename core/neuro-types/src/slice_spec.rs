...
mod tests {
    ...
    fn test_slice_full_extent() {
        ...
        assert!(
            (axial_center[0] - 0.0).abs() < 1e-6,
            "Axial X should be at origin"
        );
        assert!(
            (axial_center[1] - 0.0).abs() < 1e-6,
            "Axial Y should be at origin"
        );
        assert!(
            (axial_center[2] - center[2]).abs() < 1e-6,
            "Axial Z should match center"
        );
        ...
        assert!(
            (sagittal_center[0] - center[0]).abs() < 1e-6,
            "Sagittal X should match center"
        );
        assert!(
            (sagittal_center[1] - 0.0).abs() < 1e-6,
            "Sagittal Y should be at origin"
        );
        assert!(
            (sagittal_center[2] - 0.0).abs() < 1e-6,
            "Sagittal Z should be at origin"
        );
        ...
        assert!(
            (coronal_center[0] - 0.0).abs() < 1e-6,
            "Coronal X should be at origin"
        );
        assert!(
            (coronal_center[1] - center[1]).abs() < 1e-6,
            "Coronal Y should match center"
        );
        assert!(
            (coronal_center[2] - 0.0).abs() < 1e-6,
            "Coronal Z should be at origin"
        );
    }
}
