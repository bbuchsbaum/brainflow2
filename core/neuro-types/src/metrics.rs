...
impl VolumeComparison {
    ...
    pub fn compute_metrics(
        ...
        let (hausdorff, hausdorff_95, assd) =
            if !volume1_coords.is_empty() && !volume2_coords.is_empty() {
                compute_surface_distances(&volume1_coords, &volume2_coords, &voxel_to_world)?
            } else {
                (0.0, 0.0, 0.0)
            };
...
fn compute_surface_distances(
    ...
    // Convert to world coordinates
    let surface1_world: Vec<Point3<f32>> = surface1
        .iter()
        .map(|&[i, j, k]| {
    ...
    let surface2_world: Vec<Point3<f32>> = surface2
        .iter()
        .map(|&[i, j, k]| {
            crate::coordinates::voxel_to_world(
                Point3::new(i as f32, j as f32, k as f32),
    ...
    let percentile_95_idx =
        ((all_distances.len() as f64 * 0.95) as usize).min(all_distances.len() - 1);
