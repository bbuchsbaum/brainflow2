...
use nalgebra::{Point3, Vector3, Vector4, Rotation3, Matrix3, Unit, Matrix4};
use crate::{Result, Error, Volume, coordinates};
...
impl OrientedEllipsoid {
    ...
    pub fn new(
        ...
        // This combines rotation^(-1) and scaling by 1/radii
        let scale_inv = Matrix3::from_diagonal(&Vector3::new(
            1.0 / radii.x,
            1.0 / radii.y,
            1.0 / radii.z
        ));
...
impl OrientedEllipsoid {
    ...
    pub fn rasterize<V: VolumeRasterizer>(&self, volume: &mut V) -> Result<()> {
        ...
        let max_i = ((bbox_max_voxel.x.ceil() as isize).min(dims[0] as isize - 1) as usize + 1).min(dims[0]);
        let max_j = ((bbox_max_voxel.y.ceil() as isize).min(dims[1] as isize - 1) as usize + 1).min(dims[1]);
        let max_k = ((bbox_max_voxel.z.ceil() as isize).min(dims[2] as isize - 1) as usize + 1).min(dims[2]);
    ...
    pub fn rasterize_supersampled<V: VolumeRasterizer>(&self, volume: &mut V, samples_per_dim: u32) -> Result<()> {
        if samples_per_dim == 0 {
            return Err(Error::InvalidSliceSpec("Samples per dimension must be positive".to_string()));
        }
        ...
        let max_i = ((bbox_max_voxel.x.ceil() as isize + 1).min(dims[0] as isize - 1) as usize + 1).min(dims[0]);
        let max_j = ((bbox_max_voxel.y.ceil() as isize + 1).min(dims[1] as isize - 1) as usize + 1).min(dims[1]);
        let max_k = ((bbox_max_voxel.z.ceil() as isize + 1).min(dims[2] as isize - 1) as usize + 1).min(dims[2]);
...
mod tests {
    ...
    fn test_scanline_rasterization() {
        ...
        let mut volume1 = TestRasterVolume::new(
            [20, 20, 20],
            [1.0, 1.0, 1.0],
            [0.0, 0.0, 0.0]
        );
        
        let mut volume2 = TestRasterVolume::new(
            [20, 20, 20],
            [1.0, 1.0, 1.0],
            [0.0, 0.0, 0.0]
        );
        ...
        assert_eq!(count1, count2, "Basic and scanline methods should produce same result");
    }
    ...
    fn test_supersampled_rasterization() {
        ...
        assert!(has_partial, "Supersampling should produce partial volume effects");
    }
    ...
    fn test_rotated_ellipsoid_rasterization() {
        ...
        let filled = volume.count_nonzero();
        assert!(filled > 100, "Rotated ellipsoid should fill many voxels: {}", filled);
