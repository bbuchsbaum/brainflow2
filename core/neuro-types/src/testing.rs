...
use crate::{CompositeRequest, Result, RgbaImage, SliceProvider};
...
impl DifferentialTester {
    ...
    pub fn compare_outputs(
        ...
        let variance = abs_diffs
            .iter()
            .map(|&x| (x - mean_abs_diff).powi(2))
            .sum::<f32>()
            / abs_diffs.len() as f32;
    ...
    fn print_diff_summary(&self, test_name: &str, result: &DiffTestResult) {
        ...
        println!(
            ...
            result.max_abs_diff, self.config.max_abs_diff
        );
        println!(
            "Max Relative Diff: {:.4} (threshold: {:.4})",
            result.max_rel_diff, self.config.max_rel_diff
        );
        println!(
            "Failed Pixels: {} / {} ({:.2}%)",
...
macro_rules! assert_providers_match {
    ...
    ($provider1:expr, $provider2:expr, $request:expr, $test_name:expr) => {{
        ...
        let result = tester
            .compare_providers($provider1, $provider2, $request, $test_name)
            .expect("Failed to run differential test");

        if !result.passed {
            panic!(
                "Differential test '{}' failed: {} / {} pixels exceeded tolerance",
...
mod tests {
    use super::*;
    use crate::{LayerSpec, LayerVisual, SliceSpec, VolumeHandle};
    ...
    fn test_identical_outputs() {
        ...
        let result = tester
            .compare_providers(&provider1, &provider2, &request, "identical")
    ...
    fn test_small_differences() {
        ...
        let rgba2 = vec![
            254, 1, 0, 255, 0, 254, 1, 255, 1, 0, 254, 255, 127, 129, 128, 255,
        ];
        ...
        let tester = DifferentialTester::new();
        let result = tester
            .compare_providers(&provider1, &provider2, &request, "small_diff")
            .expect("Test should succeed");
    ...
    fn test_large_differences() {
        ...
        let rgba2 = vec![
            200, 50, 0, 255, 0, 200, 50, 255, 50, 0, 200, 255, 100, 150, 128, 255,
        ];
        ...
        let tester = DifferentialTester::new();
        let result = tester
            .compare_providers(&provider1, &provider2, &request, "large_diff")
            .expect("Test should succeed");
