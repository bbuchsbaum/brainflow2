...
This crate breaks cyclic dependencies - keep it lightweight with only shared types/traits. DO NOT add business logic here. VolumeSendable supports both 3D (DenseVolume3) and 4D (neuroim::DenseNeuroVec) variants for time series. BridgeError provides shared bridge-level error variants and conversions for bridge-owned helper errors such as VolumeMathError; renderer-specific error mapping belongs in `api_bridge`, not here. All public types should derive TS trait for TypeScript export. Run `export_types` binary after adding new types.
...
- `render_contracts` types should be consumed directly by callers that need renderer-facing view/frame contracts
