...
use crate::orthogonal_renderer::{
    add_crosshairs_to_slices, create_orthogonal_slices, OrthogonalSliceConfig, OrthogonalSlices,
};
use nalgebra::{Point3, Rotation3, Vector3};
use neuro_cpu::CpuEllipsoidRenderer;
use neuro_types::{OrientedEllipsoid, Result as NeuroResult, RgbaImage, SliceSpec};
...
impl DifferentialTestHarness {
    ...
    pub async fn run_test(
        ...
        let cpu_output = self
            .cpu_renderer
            .render_volume_slice(ellipsoid, slice_spec, color)?;
        ...
        let gpu_output = if let Some(ref mut gpu_renderer) = self.gpu_renderer {
            ...
            return Err(neuro_types::Error::TestError(
                "GPU renderer not initialized".into(),
            ));
        };
    ...
    pub async fn run_orthogonal_test(
        ...
        let (axial_spec, sagittal_spec, coronal_spec) =
            create_orthogonal_slices(world_coordinate, &config);

        // Render CPU slices
        let cpu_axial = self
            .cpu_renderer
            .render_volume_slice(ellipsoid, &axial_spec, color)?;
        let cpu_sagittal =
            self.cpu_renderer
                .render_volume_slice(ellipsoid, &sagittal_spec, color)?;
        let cpu_coronal = self
            .cpu_renderer
            .render_volume_slice(ellipsoid, &coronal_spec, color)?;
        ...
        // Render GPU slices
        let (gpu_axial, gpu_sagittal, gpu_coronal) =
            if let Some(ref mut gpu_renderer) = self.gpu_renderer {
                let axial = gpu_renderer
                    .render_volume_slice(ellipsoid, &axial_spec, color)
                    .await?;
                let sagittal = gpu_renderer
                    .render_volume_slice(ellipsoid, &sagittal_spec, color)
                    .await?;
                let coronal = gpu_renderer
                    .render_volume_slice(ellipsoid, &coronal_spec, color)
                    .await?;
    ...
    pub async fn run_comprehensive_suite(&mut self) -> NeuroResult<Vec<DifferentialTestResult>> {
        ...
        results.push(
            self.run_test("simple_sphere", &sphere, &slice, [255, 0, 0, 255])
                .await?,
        );
        ...
        results.push(
            self.run_test("elongated_ellipsoid", &ellipsoid, &slice, [0, 255, 0, 200])
                .await?,
        );
        ...
        let rotated = OrientedEllipsoid::from_euler_angles(
            ...
            0.0,
            0.0,
            std::f64::consts::PI / 4.0, // 45 degree rotation
            1.2,
        )?;
        results.push(
            self.run_test("rotated_ellipsoid", &rotated, &slice, [0, 0, 255, 180])
                .await?,
        );
        ...
        results.push(
            self.run_test("small_ellipsoid", &small, &slice, [255, 255, 0, 128])
                .await?,
        );
        ...
        results.push(
            self.run_test("large_ellipsoid", &large, &slice, [255, 0, 255, 255])
                .await?,
        );
    ...
    pub fn compute_metrics(
        &self,
        cpu_output: &RgbaImage,
        gpu_output: &RgbaImage,
    ) -> NeuroResult<DifferentialMetrics> {
    ...
    fn compute_ssim(&self, cpu_output: &RgbaImage, gpu_output: &RgbaImage) -> f64 {
        ...
        let gpu_gray: Vec<f64> = gpu_output
            .chunks(4)
            .map(|pixel| {
                0.299 * pixel[0] as f64 + 0.587 * pixel[1] as f64 + 0.114 * pixel[2] as f64
            })
    ...
    fn evaluate_metrics(&self, metrics: &DifferentialMetrics) -> (bool, Option<String>) {
        ...
        if metrics.dice_coefficient < self.dice_threshold {
            failures.push(format!(
                "Dice coefficient {:.3} < {:.3}",
                metrics.dice_coefficient, self.dice_threshold
            ));
        }

        if metrics.hausdorff_distance > self.hausdorff_threshold {
            failures.push(format!(
                "Hausdorff distance {:.1} > {:.1}",
                metrics.hausdorff_distance, self.hausdorff_threshold
            ));
        }
        ...
        if metrics.rmse > self.rmse_threshold {
            failures.push(format!(
                "RMSE {:.1} > {:.1}",
                metrics.rmse, self.rmse_threshold
            ));
        }
        ...
        if metrics.max_absolute_difference > self.max_diff_threshold {
            failures.push(format!(
                ...
                metrics.max_absolute_difference, self.max_diff_threshold
            ));
        }

        if metrics.difference_percentage > self.difference_percentage_threshold {
            failures.push(format!(
                "Difference percentage {:.1}% > {:.1}%",
                metrics.difference_percentage, self.difference_percentage_threshold
            ));
        }
    ...
    pub fn generate_report(&self, results: &[DifferentialTestResult]) -> String {
        ...
        for result in results {
            ...
            report.push_str(&format!(
                "  Dice: {:.3}, Jaccard: {:.3}, RMSE: {:.1}, SSIM: {:.3}\n",
            ...
            report.push_str(&format!(
                "  Max diff: {}, Max err: {:.1}, Diff pixels: {:.1}%\n",
