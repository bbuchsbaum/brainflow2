...
pub fn world_to_slice_pixel(world_point: Point3<f64>, slice_spec: &SliceSpec) -> (i32, i32) {
    ...
    let u_proj =
        rel_x * slice_spec.u_mm[0] + rel_y * slice_spec.u_mm[1] + rel_z * slice_spec.u_mm[2];
    let v_proj =
        rel_x * slice_spec.v_mm[0] + rel_y * slice_spec.v_mm[1] + rel_z * slice_spec.v_mm[2];
    ...
    let u_mag_sq =
        slice_spec.u_mm[0].powi(2) + slice_spec.u_mm[1].powi(2) + slice_spec.u_mm[2].powi(2);
    let v_mag_sq =
        slice_spec.v_mm[0].powi(2) + slice_spec.v_mm[1].powi(2) + slice_spec.v_mm[2].powi(2);
...
pub fn add_crosshairs_to_slices(
    ...
    draw_crosshairs(
        ...
        sag_x,
        sag_y,
        sagittal_spec.dim_px[0],
        sagittal_spec.dim_px[1],
    );
    draw_crosshairs(
        &mut slices.coronal,
        cor_x,
        cor_y,
        coronal_spec.dim_px[0],
        coronal_spec.dim_px[1],
    );
}
