    ...
    fn generate_difference_map(
        ...
        for i in (0..cpu_data.len()).step_by(4) {
            ...
            let magnitude = ((r_diff as f64).powi(2)
                + (g_diff as f64).powi(2)
                + (b_diff as f64).powi(2)
                + (a_diff as f64).powi(2))
            .sqrt();
...
"#,
                ...
                result.metrics.ssim,
                if result.metrics.psnr.is_infinite() {
                    999.0
                } else {
                    result.metrics.psnr
                },
...
"#;
        ...
        fs::write(
            format!("{}/differential_dashboard.css", self.output_dir),
            css,
        )?;
...
pub async fn run_differential_testing_with_dashboard(output_dir: &str) -> Result<String> {
    ...
    let results = match harness.run_comprehensive_suite().await {
        ...
        Err(e) => {
            println!(
                "Test suite failed: {}. Generating dashboard with available data.",
                e
            );
            Vec::new()
        }
    ...
    println!(
        "Generated differential testing dashboard: {}",
        dashboard_path
    );
    Ok(dashboard_path)
}
