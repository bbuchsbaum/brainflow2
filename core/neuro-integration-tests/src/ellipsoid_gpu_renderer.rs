...
use neuro_types::{
    CompositeRequest, LayerSpec, LayerVisual, OrientedEllipsoid, Result as NeuroResult, RgbaImage,
    SliceSpec, VolumeHandle,
};
...
impl GpuEllipsoidRenderer {
    ...
    async fn create_ellipsoid_volume(
        &mut self,
        ellipsoid: &OrientedEllipsoid,
    ) -> NeuroResult<VolumeHandle> {
        ...
        .map_err(|e| {
            neuro_types::Error::TestError(format!("Failed to create NeuroSpace: {}", e))
        })?;
        ...
        // Upload to GPU
        let (texture_idx, _transform) =
            self.render_service.upload_volume_3d(&volume).map_err(|e| {
                neuro_types::Error::TestError(format!("Failed to upload volume: {}", e))
            })?;
    ...
    pub fn cleanup_volumes(&mut self) -> NeuroResult<()> {
        ...
        for texture_idx in volumes_to_clean {
            self.render_service
                ...
                .map_err(|e| {
                    neuro_types::Error::TestError(format!(
...
mod tests {
    ...
    async fn test_gpu_ellipsoid_renderer_creation() {
        ...
        assert!(
            result.is_ok(),
            "GPU ellipsoid renderer creation failed: {:?}",
            result.err()
        );
    }
    ...
    async fn test_gpu_ellipsoid_renderer_init() {
        ...
        assert!(
            result.is_ok(),
            "GPU ellipsoid renderer initialization failed: {:?}",
            result.err()
        );
    }
