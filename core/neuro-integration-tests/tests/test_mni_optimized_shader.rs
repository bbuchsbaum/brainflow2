...
async fn test_optimized_shader_correctness() {
    ...
    let render_with_shader = |service: &mut RenderLoopService, shader_name: &str| -> Vec<u8> {
        ...
        service.update_layer_threshold(0, 0.0, 0.0) // No thresholding
    ...
    assert!(max_diff <= 255, "Maximum pixel difference {} exceeds threshold", max_diff);
    assert!(avg_diff <= 100.0, "Average pixel difference {:.2} exceeds threshold", avg_diff);
...
async fn test_optimized_shader_performance() {
    ...
    // Load volume
    let (volume_sendable, _) = load_nifti_volume_auto(&mni_path)
    ...
    service.initialize_colormap().expect("Failed to init colormap");
    service.create_world_space_bind_groups().expect("Failed to create bind groups");
    ...
    service.register_volume_with_range("perf_volume".to_string(), atlas_idx, data_range)
        .expect("Failed to register volume");
    ...
    for (width, height) in resolutions {
        ...
        let benchmark = |service: &mut RenderLoopService, shader: &str| -> f64 {
            ...
            service.clear_render_layers();
            service.add_render_layer(atlas_idx, 1.0, (0.0, 0.0, 1.0, 1.0)).unwrap();
            ...
            service.update_layer_intensity(0, intensity_range.0, intensity_range.1).unwrap();
