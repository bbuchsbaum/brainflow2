...
use neuro_integration_tests::{
    DifferentialTestHarness, OrthogonalTestResult, OrthogonalDashboard,
    add_crosshairs_to_slices, OrthogonalSlices, ImageDimensions,
};
use neuro_cpu::CpuVolumeRenderer;
use neuro_types::{RgbaImage, OrientedEllipsoid, SliceSpec, ViewRectMm, ViewOrientation, VolumeMetadata};
...
async fn test_mni_orthogonal_dashboard() {
    ...
    println!("MNI volume data range: [{:.1}, {:.1}]", data_range.0, data_range.1);
    ...
    for config in create_mni_test_configs() {
        println!("\nTesting location: {} - {}", config.name, config.description);
        println!("World coordinate: ({:.1}, {:.1}, {:.1}) mm", 
                 config.world_coordinate.x,
                 config.world_coordinate.y,
                 config.world_coordinate.z);
        ...
        println!("Axial view dimensions: {}x{}", axial_view.width_px, axial_view.height_px);
        ...
        println!("Sagittal view dimensions: {}x{}", sagittal_view.width_px, sagittal_view.height_px);
        ...
        println!("Coronal view dimensions: {}x{}", coronal_view.width_px, coronal_view.height_px);
        ...
        // Render CPU slices
        let cpu_axial = cpu_renderer.render_volume_slice(
            &volume,
            &axial_spec,
            data_range.0,
            data_range.1,
        ...
        let cpu_sagittal = cpu_renderer.render_volume_slice(
            &volume,
            &sagittal_spec,
            data_range.0,
            data_range.1,
        ...
        let cpu_coronal = cpu_renderer.render_volume_slice(
            &volume,
            &coronal_spec,
            data_range.0,
            data_range.1,
        ...
        let gpu_axial = render_gpu_slice_from_view(&mut gpu_service, volume_id.clone(), &axial_view, data_range).await;
        let gpu_sagittal = render_gpu_slice_from_view(&mut gpu_service, volume_id.clone(), &sagittal_view, data_range).await;
        let gpu_coronal = render_gpu_slice_from_view(&mut gpu_service, volume_id.clone(), &coronal_view, data_range).await;
        ...
        let axial_metrics = harness.compute_metrics(&cpu_slices.axial, &gpu_slices.axial)
            .expect("Failed to compute axial metrics");
        let sagittal_metrics = harness.compute_metrics(&cpu_slices.sagittal, &gpu_slices.sagittal)
            .expect("Failed to compute sagittal metrics");
        let coronal_metrics = harness.compute_metrics(&cpu_slices.coronal, &gpu_slices.coronal)
        ...
        let sagittal_passed = sagittal_metrics.dice_coefficient >= 0.9 && sagittal_metrics.ssim >= 0.85;
        let coronal_passed = coronal_metrics.dice_coefficient >= 0.9 && coronal_metrics.ssim >= 0.85;
        
        let passed = axial_passed && sagittal_passed && coronal_passed;
        let mut failure_reasons = Vec::new();
        if !axial_passed {
            failure_reasons.push(format!("Axial plane: Dice={:.3}, SSIM={:.3}", 
                axial_metrics.dice_coefficient, axial_metrics.ssim));
        }
        if !sagittal_passed {
            failure_reasons.push(format!("Sagittal plane: Dice={:.3}, SSIM={:.3}", 
                sagittal_metrics.dice_coefficient, sagittal_metrics.ssim));
        }
        if !coronal_passed {
            failure_reasons.push(format!("Coronal plane: Dice={:.3}, SSIM={:.3}", 
        ...
        let result = OrthogonalTestResult {
            ...
            ellipsoid: OrientedEllipsoid::sphere(
                Point3::new(0.0, 0.0, 0.0), 1.0, 1.0
            ).unwrap(), // Dummy ellipsoid for compatibility
            world_coordinate: config.world_coordinate,
    ...
    let dashboard_path = dashboard.generate_dashboard(&test_results).await
...
async fn render_gpu_slice_from_view(
    ...
    use render_loop::view_state::{ViewState, ViewId};
    ...
    let result = service.request_frame(ViewId::new("orthogonal_test"), view_state)
        .await
