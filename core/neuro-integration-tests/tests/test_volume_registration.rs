...
async fn test_register_volume_with_upload() {
    ...
    use render_loop::view_state::{ViewState, CameraState, SliceOrientation, LayerConfig};
    ...
    gpu_service.set_view_state(&multi_volume_state)
...
async fn test_invalid_volume_id() {
    ...
    if let Err(e) = result {
        ...
        assert!(format!("{:?}", e).contains("not registered"), 
