...
use neuro_types::{SliceSpec, OrientedEllipsoid};
...
async fn test_simple_sphere_differential() {
    ...
    if let Err(e) = harness.init_gpu().await {
        println!("GPU initialization failed: {}. Skipping GPU comparison tests.", e);
        return;
    }
    ...
    match result {
        Ok(test_result) => {
            ...
            println!("Dice coefficient: {:.3}", test_result.metrics.dice_coefficient);
            println!("RMSE: {:.1}", test_result.metrics.rmse);
            println!("Max pixel difference: {}", test_result.metrics.max_absolute_difference);
            println!("Differing pixels: {:.1}%", test_result.metrics.difference_percentage);
...
async fn test_comprehensive_differential_suite() {
    ...
    if let Err(e) = harness.init_gpu().await {
        println!("GPU initialization failed: {}. Skipping comprehensive tests.", e);
        return;
    }
...
async fn test_rotated_ellipsoid_cpu() {
    ...
    let rotated = OrientedEllipsoid::from_euler_angles(
        ...
        0.0, 0.0, std::f64::consts::PI / 4.0, // 45 degree rotation around Z
        1.0,
    ...
    match result {
        Ok(image_data) => {
            ...
            let visible_pixels = image_data.chunks(4)
                .filter(|pixel| pixel[3] > 10) // Alpha > 10
                .count();
            
            println!("Visible pixels: {}", visible_pixels);
            assert!(visible_pixels > 500, "Rotated ellipsoid should be clearly visible");
            
            // Check for yellow color in visible pixels
            let yellow_pixels = image_data.chunks(4)
...
fn test_differential_metrics_computation() {
    ...
    let metrics2 = harness.compute_metrics(&image1, &image3)
        .expect("Failed to compute metrics");
    ...
    assert!(metrics2.rmse > 0.0, "RMSE should be > 0 for different images");
    assert_eq!(metrics2.max_absolute_difference, 2, "Max difference should be 2");
}