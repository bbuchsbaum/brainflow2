...
async fn test_single_roi_overlay() {
    ...
    let roi_volume =
        create_spherical_roi_volume(mni_volume.space(), roi_center_mm, roi_radius_mm, roi_value)
    ...
    let roi_voxel_count = roi_volume.values().iter().filter(|&&v| v > 0.0).count();
    ...
    for (orientation, name) in test_orientations {
        ...
        let mni_img =
            image::RgbaImage::from_raw(view_rect.width_px, view_rect.height_px, mni_only.clone())
                .expect("Failed to create MNI image");
        let mni_path = dashboard_dir.join(format!("mni_only_{}.png", name));
        mni_img.save(&mni_path).expect("Failed to save MNI image");
        ...
        let overlay_result = gpu_service
            .request_frame(ViewId::new("overlay_view"), overlay_state)
            .await
            .expect("Failed to render overlay");

        // Save overlay image using actual view rect dimensions
        let overlay_img = image::RgbaImage::from_raw(
            view_rect.width_px,
            view_rect.height_px,
            overlay_result.image_data.clone(),
        )
        ...
        let roi_pixels = overlay_result
            .image_data
            .chunks(4)
            .filter(|pixel| {
                if pixel[0] > 100 && pixel[3] == 255 {
                    // Any red component with full alpha
    ...
    println!(
        "   Dashboard images saved to: {:?}",
        dashboard_dir.canonicalize().ok()
    );
}
...
async fn test_multiple_roi_overlay() {
    ...
    for (i, (center, radius, name)) in roi_specs.iter().enumerate() {
        println!(
            "  Creating ROI {}: {} at {:?}, radius {} mm",
            i, name, center, radius
        );

        let roi = create_spherical_roi_volume(mni_volume.space(), *center, *radius, 1.0)
            .expect("Failed to create ROI");

        let roi_id = format!("roi_{}", i);
        gpu_service
            .register_volume_with_upload(roi_id, &roi, wgpu::TextureFormat::R32Float)
    ...
    println!(
        "\nRendering multi-layer view with {} ROIs...",
        roi_specs.len()
    );
    let result = gpu_service
        .request_frame(ViewId::new("multi_roi_view"), multi_state)
        .await
    ...
    multi_img.save(&multi_path).expect("Failed to save image");

    println!("  Saved multi-ROI overlay to: {:?}", multi_path);
    println!(
        "  Rendered {} layers successfully",
        result.rendered_layers.len()
    );

    assert_eq!(
        result.rendered_layers.len(),
        6,
        "Should render MNI + 5 ROIs"
    );
...
async fn test_overlapping_roi_transparency() {
    ...
    let roi1 = create_spherical_roi_volume(mni_volume.space(), roi1_center, overlap_radius, 1.0)
        .expect("Failed to create ROI 1");

    let roi2 = create_spherical_roi_volume(mni_volume.space(), roi2_center, overlap_radius, 1.0)
    ...
    gpu_service
        .register_volume_with_upload("roi1".to_string(), &roi1, wgpu::TextureFormat::R32Float)
        .expect("Failed to register ROI 1");

    gpu_service
        .register_volume_with_upload("roi2".to_string(), &roi2, wgpu::TextureFormat::R32Float)
    ...
    for (blend_mode, blend_name, opacity) in blend_tests {
        println!(
            "\nTesting {} blend mode with opacity {}...",
            blend_name, opacity
        );
        ...
        let result = gpu_service
            .request_frame(ViewId::new("blend_test"), state)
            .await
