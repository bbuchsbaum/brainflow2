...
async fn benchmark_shader_render(
    ...
    for _ in 0..warmup_runs {
        let _ = service.render_to_buffer().expect("Warmup render failed");
    }
    ...
    for _ in 0..benchmark_runs {
        ...
        let _ = service.render_to_buffer().expect("Benchmark render failed");
...
async fn benchmark_standard_vs_optimized_shader() {
    ...
    service
        .register_volume_with_range("benchmark_volume".to_string(), atlas_idx, data_range)
    ...
    println!(
        "Running {} iterations for detailed analysis...",
        num_detailed_runs
    );
    ...
    println!(
        "  Improvement: {:.1}%",
        ((std_mean - opt_mean) / std_mean) * 100.0
    );
...
async fn benchmark_shader_with_multiple_layers() {
    ...
    // Load volume
    let (volume_sendable, _transform) =
        load_nifti_volume_auto(&mni_path).expect("Failed to load NIfTI");
    ...
    for layer_count in 1..=num_layers {
        ...
        let benchmark_multi = |service: &mut RenderLoopService, shader: &str| -> f64 {
            ...
            for i in 0..layer_count {
                ...
                service
                    .update_layer_intensity(i, intensity_range.0, intensity_range.1)
                    .unwrap();
        ...
        println!(
            "{:<10} {:>8.3} {:>20.3} {:>20.2}x",
            layer_count, std_mean, opt_mean, speedup
        );
    }
...
async fn profile_shader_differences() {
    ...
    service
        .register_volume_with_range("profile_volume".to_string(), atlas_idx, data_range)
        .expect("Failed to register volume");
    ...
    println!(
        "  Optimized: {:.3} ms (speedup: {:.2}x)",
        opt_time_thresh,
        std_time_thresh / opt_time_thresh
    );
    ...
    println!(
        "  Optimized: {:.3} ms (speedup: {:.2}x)",
        opt_time_alpha,
        std_time_alpha / opt_time_alpha
    );

    println!("\n🔍 Performance Analysis:");
    println!(
        "LOD sampling overhead: {:.1}%",
        ((opt_time - std_time) / std_time) * 100.0
    );
    println!(
        "Threshold optimization impact: {:.1}%",
        ((opt_time_thresh - std_time_thresh) / std_time_thresh) * 100.0
    );
    println!(
        "Early exit benefit: {:.1}%",
...
async fn profile_shader_scenario(
    ...
    service.clear_render_layers();
    service
        .add_render_layer(atlas_idx, opacity, (0.0, 0.0, 1.0, 1.0))
        .expect("Failed to add render layer");
    service
        .set_layer_colormap(0, 0)
        .expect("Failed to set colormap");
    ...
    if use_threshold {
        ...
        service
            .update_layer_threshold(0, thresh_low, thresh_high)
            .expect("Failed to update threshold");
    } else {
        service
            .update_layer_threshold(0, 0.0, 0.0)
