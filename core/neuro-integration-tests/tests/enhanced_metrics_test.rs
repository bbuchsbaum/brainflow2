...
fn test_enhanced_metrics_identical_images() {
    ...
    assert_eq!(
        metrics.difference_percentage, 0.0,
        "Difference percentage should be 0"
    );
    assert!(
        metrics.psnr.is_infinite(),
        "PSNR should be infinity for perfect match"
    );
    assert!(
        (metrics.ssim - 1.0).abs() < 1e-10,
    ...
    assert!(
        (metrics.dice_coefficient - 1.0).abs() < 1e-10,
        "Dice should be 1.0 for identical images"
    );
    assert!(
        (metrics.jaccard_index - 1.0).abs() < 1e-10,
        "Jaccard should be 1.0 for identical images"
    );
}
...
fn test_enhanced_metrics_different_images() {
    ...
    assert!(
        metrics.psnr.is_finite() && metrics.psnr > 0.0,
    ...
    assert!(
        metrics.dice_coefficient >= 0.0 && metrics.dice_coefficient <= 1.0,
        "Dice coefficient should be in [0,1]"
    );
    assert!(
        metrics.jaccard_index >= 0.0 && metrics.jaccard_index <= 1.0,
        "Jaccard index should be in [0,1]"
    );
    assert!(
        metrics.jaccard_index <= metrics.dice_coefficient,
    ...
    assert!(
        ...
        "Dice should be > 0 for overlapping shapes"
    );
    assert!(
        metrics.jaccard_index > 0.0,
        "Jaccard should be > 0 for overlapping shapes"
    );
}
...
fn test_enhanced_metrics_no_overlap() {
    ...
    assert_eq!(
        metrics.jaccard_index, 0.0,
        "Jaccard should be 0 for no overlap"
    );
}
...
fn test_enhanced_metrics_mathematical_relationships() {
    ...
    for (name, image1, image2) in test_cases {
        ...
        assert_eq!(
            metrics.max_absolute_error, metrics.max_absolute_difference as f64,
            "Max absolute error should equal max absolute difference for {}",
            name
        );

        // Jaccard <= Dice (mathematical relationship)
        assert!(
            metrics.jaccard_index <= metrics.dice_coefficient + 1e-10,
        ...
        assert!(
            metrics.ssim >= -1.0 && metrics.ssim <= 1.0,
            "SSIM ({:.6}) should be in [-1, 1] for {}",
            metrics.ssim,
            name
        );
    }
