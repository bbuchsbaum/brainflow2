...
async fn test_baseline_single_volume_imperative() {
    ...
    let output_dir =
        workspace_root.join("core/neuro-integration-tests/test_output/baseline_imperative");
    ...
    for (orientation, name) in [
        ...
        gpu_service
            .create_offscreen_target(viewport_size[0], viewport_size[1])
        ...
        let (origin, u_vec, v_vec) = match orientation {
            "axial" => (
                [
                    crosshair_world[0] - 100.0,
                    crosshair_world[1] - 100.0,
                    crosshair_world[2],
                    1.0,
                ],
                [200.0, 0.0, 0.0, 0.0],
        ...
        let img =
            image::RgbaImage::from_raw(viewport_size[0], viewport_size[1], image_data.clone())
...
async fn test_baseline_multi_layer_imperative() {
    ...
    let (atlas_idx1, _) = gpu_service
        .upload_volume_3d(&volume)
        .expect("Failed to upload volume 1");
    let (atlas_idx2, _) = gpu_service
        .upload_volume_3d(&volume)
    ...
    // Layer 1: Base layer
    gpu_service
        .add_render_layer(atlas_idx1, 0.7, (0.0, 0.0, 1.0, 1.0))
    ...
    gpu_service
        .add_render_layer(atlas_idx2, 0.3, (0.0, 0.0, 1.0, 1.0))
        .expect("Failed to add layer 2");
    gpu_service
        .set_layer_colormap(1, 2) // Use hot colormap
        .expect("Failed to set colormap");
    gpu_service
        .update_layer_intensity(1, data_range.0 * 0.5, data_range.1 * 0.8)
        .expect("Failed to update intensity");
