...
use neuro_integration_tests::{SimpleTestRunner, SimpleVisualDashboard};
...
fn test_generate_visual_dashboard() {
    ...
    assert!(
        std::path::Path::new("test_dashboard_output/dashboard.css").exists(),
        "CSS file should exist"
    );
    assert!(
        std::path::Path::new("test_dashboard_output/dashboard.js").exists(),
...
fn test_generate_dashboard_with_custom_output() {
    ...
    let output_dir =
        std::env::var("DASHBOARD_OUTPUT").unwrap_or_else(|_| "custom_dashboard_output".to_string());

    // Create test results
    let test_results = vec![SimpleTestResult {
