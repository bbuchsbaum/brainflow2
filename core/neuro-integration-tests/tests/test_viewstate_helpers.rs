...
use render_loop::view_state::{ViewState, SliceOrientation, LayerConfig};
...
fn test_multi_layer_viewstate() {
    ...
    // Create base layer
    let base_layer = LayerConfig::new("anatomical".to_string())
        .with_intensity_window(0.0, 255.0);
...
fn test_viewstate_validation() {
    ...
    let mut view_state = ViewState::default_for_volume("test".to_string(), [256, 256, 128])
        .with_viewport(0, 512);
    
    let result = view_state.validate();
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("Viewport dimensions must be non-zero"));
