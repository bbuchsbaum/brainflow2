...
fn bench_validation_metrics(c: &mut Criterion) {
    use neuro_types::{Volume, VolumeComparison};
    ...
    for (dimensions, size_name) in sizes {
        ...
        let volume1 = MockVolume {
            dimensions,
            data: data1,
        };
        let volume2 = MockVolume {
            dimensions,
            data: data2,
        };
        ...
        group.bench_with_input(
            ...
            &(volume1, volume2),
            |b, (vol1, vol2)| {
                b.iter(|| VolumeComparison::compute_metrics(vol1, vol2, 50.0, None).unwrap());
            },
        );
    }
...
fn bench_integration_tests(c: &mut Criterion) {
    ...
    for (i, config) in configs.iter().take(3).enumerate() {
        // Only test first 3 configs
        group.bench_with_input(
            BenchmarkId::new("full_test", format!("config_{}", i)),
            config,
            |b, config| {
                b.iter(|| runner.run_ellipsoid_test(config));
            },
        );
    }
