...
use neuro_integration_tests::{
    IntegrationVolumeStore, TestLayerGenerator, TestRequestGenerator, TestSliceGenerator,
};
use std::sync::Arc;
...
fn benchmark_axial_slices(c: &mut Criterion) {
    ...
    let volume_store = Arc::new(
        IntegrationVolumeStore::with_standard_volumes().expect("Failed to create test volumes"),
    );
...
fn benchmark_multi_layer_compositing(c: &mut Criterion) {
    ...
    // Two layers
    let anat_layer = TestLayerGenerator::windowed_layer(0, Matrix4::identity(), 0.0, 1000.0);
    ...
    let layer3 = TestLayerGenerator::colormap_layer(
        1,
        Matrix4::identity(),
        neuro_types::Colormap::Cool,
        0.5,
    );
    let layer4 = TestLayerGenerator::colormap_layer(
        3,
        Matrix4::identity(),
        neuro_types::Colormap::Viridis,
        0.3,
    );
...
fn benchmark_interpolation_methods(c: &mut Criterion) {
    ...
    let mut slice_nearest =
        TestSliceGenerator::oblique_slice([5.5, -10.3, 20.7], 0.3, 0.6, [256.0, 256.0], [256, 256]);
