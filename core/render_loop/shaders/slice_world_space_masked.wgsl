...
fn sampleLayer(layer: LayerData, world_mm: vec3<f32>) -> vec4<f32> {
    ...
    // Apply thresholding based on mode. For debugging alpha issues, we start from
    // a fixed alpha of 1.0 instead of reading layer.opacity from the UBO; when
    // SLICE_DISABLE_THRESHOLD_DEBUG == 1u, thresholding is skipped and alpha
    // remains equal to layer.opacity for all layers.
