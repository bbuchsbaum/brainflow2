...
impl RenderTargetPool {
    ...
    pub fn get_current_target(&self, key: &RenderTargetKey) -> Option<(&wgpu::Texture, &wgpu::TextureView)> {
        self.cache.get(key).map(|entry| (&entry.texture, &entry.view))
    }
    ...
    fn evict_oldest(&mut self) {
        if let Some(oldest_key) = self.lru_order.pop_front() {
            ...
            log::info!("Evicted render target: {}x{}", oldest_key.width, oldest_key.height);
        }
    }
