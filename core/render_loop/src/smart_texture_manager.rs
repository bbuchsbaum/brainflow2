...
use volmath::{DataRange, DenseVolume3, NeuroSpaceExt, NumericType, VoxelData};
...
impl SmartTextureManager {
            ...
            NumericType::U16 | NumericType::I16 => {
                ...
                if let Some((min, max)) = volume.range() {
                    if let (Some(min_f32), Some(max_f32)) = (
                        num_traits::cast::<T, f32>(min),
                        num_traits::cast::<T, f32>(max),
                    ) {
    ...
    fn find_pooled_texture(
        ...
        let position = self
            .texture_pool
            .iter()
            .position(|pooled| pooled.dimensions == dimensions && pooled.format == format);
            ...
            + std::ops::Mul<Output = T>,
    {
        ...
        if self.total_memory_usage + memory_size > self.memory_limit {
            return Err(RenderLoopError::Internal {
                code: 6005,
                details: format!(
                    "Memory limit exceeded. Required: {} MB, Available: {} MB",
        ...
        let (index, texture, view) =
            if let Some((idx, pooled)) = self.find_pooled_texture(dimensions, format) {
...
mod tests {
    ...
    fn test_memory_calculation() {
        assert_eq!(
            SmartTextureManager::calculate_memory_size(
                [256, 256, 256],
                wgpu::TextureFormat::R8Unorm
            ),
            16_777_216 // 16 MB
    ...
    fn test_texture_pooling() {
        ...
        pollster::block_on(async {
            ...
            // Release the texture
            manager
                .release_texture(index1)
                .expect("Failed to release texture");
    ...
    fn test_memory_limit() {
        ...
        pollster::block_on(async {
            ...
            let result = manager.upload_volume(&device, &queue, &volume, None);
