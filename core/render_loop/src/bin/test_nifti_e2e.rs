...
async fn test_synthetic_volume_render_to_png() {
    ...
    service
        .set_layer_colormap(0, 1) // Hot colormap for better visibility
        .expect("Failed to set colormap");
    service
        .update_layer_intensity(0, 0.0, 1000.0)
        .expect("Failed to update intensity");
    service
        .update_layer_threshold(0, -f32::INFINITY, f32::INFINITY)
    ...
    println!(
        "  Non-background pixels: {} ({:.1}% coverage)",
        non_background_count, coverage
    );
