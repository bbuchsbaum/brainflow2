...
use render_loop::{
    view_state::{SliceOrientation, ViewId, ViewState},
    RenderLoopService,
};
use std::fs;
use std::path::Path;
...
async fn main() {
    ...
    if !mni_path.exists() {
        ...
        println!(
            "MNI file not found at {:?}, using synthetic volume",
            mni_path
        );
...
async fn test_synthetic_brain_slices() {
    ...
    service
        .register_volume_with_upload(volume_id.clone(), &volume, wgpu::TextureFormat::R32Float)
...
async fn extract_axial_slices(
    ...
    while z <= superior_z {
        ...
        println!(
            "  Slice {:03} at Z={:.1}mm: {:.1}% coverage, {} colors",
            slice_count, z, coverage, unique_colors
        );

        // Save individual slice
        let slice_path =
            output_dir.join(format!("{}_slice_{:03}_z{:.1}.png", prefix, slice_count, z));
...
fn create_slice_montage(
    ...
    println!(
        "\nCreating montage: {} slices in {}x{} grid",
        slice_count, grid_cols, grid_rows
    );
...
fn create_synthetic_brain_volume() -> DenseVolume3<f32> {
    ...
    let space_impl =
        <volmath::NeuroSpace as NeuroSpaceExt>::from_dims_spacing_origin(dims, spacing, origin)
