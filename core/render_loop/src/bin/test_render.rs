...
use render_loop::{BlendMode, FrameTimeTracker, LayerInfo, RenderLoopService, ThresholdMode};
...
fn create_volume_from_data(
    ...
    let space_impl =
        <volmath::NeuroSpace as NeuroSpaceExt>::from_affine_matrix4(dims_vec, transform)
...
async fn render_views(
    ...
    for (view_name, u_vec, v_vec) in views {
        ...
        service.update_frame_ubo([0.0, 0.0, 0.0, 1.0], u_vec, v_vec);
        ...
        let filename = format!(
            "{}_{}.png",
            args.pattern.to_string().to_lowercase(),
            view_name
        );
