...
impl SelectedSliceRenderer {
    ...
    pub fn new(
        ...
        pipeline_layout: &wgpu::PipelineLayout,
        surface_format: wgpu::TextureFormat,
        #[allow(unused_variables)] use_precomputed: bool,
    ) -> Result<Self, crate::RenderLoopError> {
        #[cfg(feature = "precomputed-shader")]
        {
            if use_precomputed {
                return Ok(Self::Precomputed(SliceRenderer::<PrecomputedSlice>::new(
                    device,
                    pipeline_layout,
                    surface_format,
                )?));
        ...
        Ok(Self::Standard(SliceRenderer::<StandardSlice>::new(
            device,
            pipeline_layout,
            surface_format,
        )?))
