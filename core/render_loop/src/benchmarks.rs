...
impl PerformanceComparison {
    ...
    pub fn print_summary(&self) {
        ...
        println!("{}: {:.2}ms ({:.1} FPS)", self.baseline_name, self.baseline_ms, self.baseline_fps);
        println!("{}: {:.2}ms ({:.1} FPS)", self.optimized_name, self.optimized_ms, self.optimized_fps);
        println!("Improvement: {:.1}% ({:.2}x speedup)", self.improvement_percent, self.speedup_factor);
...
mod tests {
    ...
    fn test_performance_comparison() {
        ...
        let comparison = PerformanceComparison::from_trackers(
            "Baseline",
            &baseline,
            "Optimized",
            &optimized,
