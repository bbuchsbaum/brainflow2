...
fn test_volume_renders_not_black() {
    pollster::block_on(async {
        ...
        // Upload volume
        let (layer_index, _transform) = service
        ...
        let world_to_voxel = Matrix4::new(
            1.0, 0.0, 0.0, 32.0, 0.0, 1.0, 0.0, 32.0, 0.0, 0.0, 1.0, 12.5, 0.0, 0.0, 0.0, 1.0,
        );
...
fn test_binary_mask_renders_correctly() {
    pollster::block_on(async {
        ...
        let world_to_voxel = Matrix4::new(
            1.0, 0.0, 0.0, 32.0, 0.0, 1.0, 0.0, 32.0, 0.0, 0.0, 1.0, 12.0, 0.0, 0.0, 0.0, 1.0,
        );
        ...
        let image_data = service.render_to_buffer().expect("Failed to render mask");
...
fn test_crosshair_position() {
    pollster::block_on(async {
        ...
        let (layer_index, _transform) =
            service.upload_volume_3d(&volume).expect("Failed to upload");
...
fn create_axial_frame_ubo(center: Vector3<f32>, width: u32, height: u32) -> FrameUbo {
    ...
    FrameUbo {
        origin_mm: [center.x - fov / 2.0, center.y - fov / 2.0, center.z, 1.0],
...
fn assert_binary_pattern_visible(image_data: &[u8], width: u32, height: u32) {
    ...
    println!(
        "Binary mask - On: {}, Off: {}, Total: {}",
        on_pixels, off_pixels, total_pixels
    );
