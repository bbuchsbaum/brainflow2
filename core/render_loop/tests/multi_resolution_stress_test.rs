...
mod tests {
    ...
    fn create_test_volume(dims: [usize; 3], spacing: [f32; 3]) -> DenseVolume3<f32> {
        ...
        let transform = Matrix4::new(
            ...
            0.0,
            0.0,
            -(dims[0] as f32 * spacing[0] / 2.0),
            0.0,
            spacing[1],
            0.0,
            -(dims[1] as f32 * spacing[1] / 2.0),
            0.0,
            0.0,
            spacing[2],
            -(dims[2] as f32 * spacing[2] / 2.0),
            0.0,
            0.0,
            0.0,
            1.0,
    ...
    fn test_many_volumes_stress() {
        pollster::block_on(async {
            ...
            service
                .enable_smart_texture_management(1024)
            ...
            let layers: Vec<LayerInfo> = volume_indices
                .iter()
                .enumerate()
                .map(|(i, &idx)| {
                    LayerInfo {
            ...
            service
                .create_offscreen_target(1024, 1024)
    ...
    fn test_memory_limit_with_pooling() {
        pollster::block_on(async {
            ...
            service
                .enable_smart_texture_management(64)
                .expect("Failed to enable smart texture management");
            ...
            let idx1 = service
                .upload_volume_smart(&large_volume1, Some(wgpu::TextureFormat::R32Float))
                .unwrap()
                .0;
            let _idx2 = service
                .upload_volume_smart(&large_volume2, Some(wgpu::TextureFormat::R32Float))
                .unwrap()
                .0;
            ...
            let result =
                service.upload_volume_smart(&large_volume3, Some(wgpu::TextureFormat::R32Float));
            ...
            let _idx3 = service
                .upload_volume_smart(&large_volume3, Some(wgpu::TextureFormat::R32Float))
                .unwrap()
                .0;
