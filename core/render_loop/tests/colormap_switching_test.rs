...
async fn test_grayscale_colormap(service: &mut RenderLoopService, size: u32) {
    ...
    service.update_layer_threshold(0, 0.0, 1.0)
        .expect("Failed to update layer threshold");
    
    // Set grayscale colormap
    service.set_layer_colormap(0, BuiltinColormap::Grayscale as u32)
...
async fn test_jet_colormap(service: &mut RenderLoopService, size: u32) {
    ...
    // Set jet colormap
    service.set_layer_colormap(0, BuiltinColormap::Jet as u32)
        .expect("Failed to set colormap");
...
async fn test_viridis_colormap(service: &mut RenderLoopService, size: u32) {
    ...
    service.set_layer_colormap(0, BuiltinColormap::Viridis as u32)
        .expect("Failed to set colormap");
...
fn verify_grayscale(buffer: &[u8], size: u32) {
    ...
    println!("  Total non-black pixels in image: {}/{}", non_black_pixels, size * size);
    ...
    println!("  Grayscale pixels: {}/{} ({:.1}%)", 
             grayscale_count, total_pixels, grayscale_ratio * 100.0);
...
fn verify_viridis_colormap(buffer: &[u8], size: u32) {
    ...
    assert!(g1 > 0 && g2 > 0, "Viridis should have green component throughout");
}
...
fn verify_threshold_colormap(buffer: &[u8], size: u32) {
    ...
    println!("  Threshold colormap - Center RGB({}, {}, {})", center_r, center_g, center_b);
    
    assert!(center_r > 0 || center_g > 0 || center_b > 0, 
