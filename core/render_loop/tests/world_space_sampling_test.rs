...
fn test_multi_texture_upload() {
    pollster::block_on(async {
        ...
        let (anat_idx, anat_tfm) = texture_manager.upload_volume(
            &device,
            &queue,
            &volumes.anatomical,
        ...
        let (func_idx, func_tfm) = texture_manager.upload_volume(
            &device,
            &queue,
            &volumes.functional,
        ...
        let (detail_idx, detail_tfm) = texture_manager.upload_volume(
            &device,
            &queue,
...
fn test_world_space_coordinate_validation() {
    ...
    let anat_valid = TransformValidator::validate_world_point(
        &world_origin,
        &anat_tfm,
        [256, 256, 256],
    );
    ...
    let func_valid = TransformValidator::validate_world_point(
        &world_origin,
        &func_tfm,
        [128, 128, 32],
    );
    ...
    let detail_valid = TransformValidator::validate_world_point(
        &world_origin,
        &detail_tfm,
        [128, 128, 64],
    );
