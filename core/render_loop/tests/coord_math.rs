...
fn world_to_voxel_with_axis_flip() {
    ...
    for world in test_points {
        ...
        if in_bounds {
            assert!(
                voxel[0] >= 0.0,
                "Voxel X should be non-negative for in-bounds world coordinate"
            );
            assert!(
                voxel[1] >= 0.0,
                "Voxel Y should be non-negative for in-bounds world coordinate"
            );
            assert!(
                voxel[2] >= 0.0,
                "Voxel Z should be non-negative for in-bounds world coordinate"
            );
            assert!(
                voxel[0] <= dims[0] as f32,
                "Voxel X should be within volume dimensions"
            );
            assert!(
                voxel[1] <= dims[1] as f32,
                "Voxel Y should be within volume dimensions"
            );
            assert!(
                voxel[2] <= dims[2] as f32,
                "Voxel Z should be within volume dimensions"
            );
        }
    }
}
