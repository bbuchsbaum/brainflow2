    ...
    fn create_marked_volume(
        ...
        for (world_pos, value) in marker_positions {
            ...
            if x >= 0 && x < dims[0] as i32 &&
               y >= 0 && y < dims[1] as i32 &&
               z >= 0 && z < dims[2] as i32 {
                let idx = (z as usize) * dims[0] * dims[1] + 
                         (y as usize) * dims[0] + 
                         (x as usize);
                data[idx] = *value;
            }
    ...
    fn test_different_orientations_alignment() {
        pollster::block_on(async {
            ...
            let transform2 = Matrix4::new(
                cos45, -sin45, 0.0, -64.0,
                sin45, cos45, 0.0, -64.0,
                0.0, 0.0, 2.0, -64.0,  // Also different Z scaling
                0.0, 0.0, 0.0, 1.0,
            );
    ...
    fn test_volume_boundary_coordinates() {
        pollster::block_on(async {
            ...
            for pos in boundary_positions {
                ...
                let voxel_pos = world_to_voxel.transform_point(&Point3::from(Vector3::from(pos)));
                let in_bounds = voxel_pos.x >= 0.0 && voxel_pos.x < dims[0] as f32 &&
                               voxel_pos.y >= 0.0 && voxel_pos.y < dims[1] as f32 &&
                               voxel_pos.z >= 0.0 && voxel_pos.z < dims[2] as f32;
