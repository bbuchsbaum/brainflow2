...
async fn test_coordinate_inversion_axial() {
    ...
    let screen_size = 256;
    service
        .create_offscreen_target(screen_size, screen_size)
    ...
    println!(
        ...
        bright_pos.0, bright_pos.1, bright_pixel
    );
    ...
    assert!(
        world_coord[0] >= -1.0 && world_coord[0] <= 65.0,
        "World X coordinate {} should be within view bounds",
        world_coord[0]
    );
    assert!(
        world_coord[1] >= -1.0 && world_coord[1] <= 65.0,
        "World Y coordinate {} should be within view bounds",
        world_coord[1]
    );
    assert!(
        (world_coord[2] - 32.0).abs() < 1.0,
        "World Z coordinate {} should be close to slice Z=32",
        world_coord[2]
    );
