...
fn test_world_space_rendering_produces_output() {
    pollster::block_on(async {
        ...
        println!("Rendered {} non-zero pixels out of {}", non_zero_pixels, 512 * 512);
        assert!(non_zero_pixels > 1000, "Expected at least 1000 non-zero pixels, got {}", non_zero_pixels);
        
        // Check that we have some variation in the image
        let unique_colors: std::collections::HashSet<[u8; 4]> = image_data.chunks(4)
            .map(|chunk| [chunk[0], chunk[1], chunk[2], chunk[3]])
            .collect();
        
        println!("Found {} unique colors in rendered image", unique_colors.len());
        assert!(unique_colors.len() >= 3, "Expected at least 3 unique colors, got {}", unique_colors.len());
    });
}
...
fn test_multi_resolution_overlay() {
    pollster::block_on(async {
        ...
        let (anat_idx, anat_tfm) = service.upload_volume_3d(&volumes.anatomical)
            .expect("Failed to upload anatomical");
        
        let (func_idx, func_tfm) = service.upload_volume_3d(&volumes.functional)
            .expect("Failed to upload functional");
        
        let (detail_idx, detail_tfm) = service.upload_volume_3d(&volumes.detail_patch)
        ...
        for z_offset in [-50.0, 0.0, 50.0].iter() {
            ...
            println!("Slice at z={}: {} non-zero pixels", z_offset, non_zero_pixels);
            assert!(non_zero_pixels > 100, "Slice at z={} has too few non-zero pixels", z_offset);
        }
    });
}