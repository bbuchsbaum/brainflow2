...
async fn test_affine_transform_in_layer_ubo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        (world_to_voxel[(0, 0)] - expected_w2v[(0, 0)]).abs() < 1e-6,
        "Rotation component incorrect"
    );
    assert!(
        (world_to_voxel[(0, 1)] - expected_w2v[(0, 1)]).abs() < 1e-6,
        "Rotation component incorrect"
    );
    assert!(
        (world_to_voxel[(1, 0)] - expected_w2v[(1, 0)]).abs() < 1e-6,
        "Rotation component incorrect"
    );
    assert!(
        (world_to_voxel[(1, 1)] - expected_w2v[(1, 1)]).abs() < 1e-6,
        "Rotation component incorrect"
    );
...
async fn test_volume_space_transforms() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..4 {
        for j in 0..4 {
            ...
            assert!(
                (identity_check[(i, j)] - expected).abs() < 1e-5,
                "Transform inverse check failed at ({}, {})",
                i,
                j
            );
        }
