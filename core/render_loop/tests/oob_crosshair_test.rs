...
async fn oob_crosshair_yields_constant() {
    ...
    service
        .update_layer_intensity(render_idx, 0.0, 200.0)
        .unwrap();
    ...
    for world_pos in oob_positions {
        ...
        assert!(
            is_oob,
            "Position {:?} should map to out-of-bounds voxel coordinates, got {:?}",
            world_pos, voxel
        );
        ...
        assert!(
            uvw_oob,
            "UVW coordinates {:?} should be out of bounds for voxel {:?}",
            uvw, voxel
        );
...
async fn oob_with_transform() {
    ...
    for world_pos in oob_world_positions {
        ...
        assert!(
            is_oob,
            "World position {:?} should be out of bounds, voxel: {:?}",
            world_pos, voxel
        );
    }
}
