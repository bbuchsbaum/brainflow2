...
fn test_debug_shader_world_coordinate_variation() {
    ...
    let mut service =
        pollster::block_on(RenderLoopService::new()).expect("Failed to create RenderLoopService");
    ...
    service.update_layer_uniforms_direct(&[layer_info], &[(64, 64, 64)], &[Matrix4::identity()]);

    // Switch to debug shader
    service
        .set_shader("slice_debug")
        .expect("Failed to switch to debug shader");

    // Render with debug shader
    let debug_image = service.render_to_buffer().expect("Failed to render");

    println!(
        "Debug shader render complete. Image size: {} bytes",
        debug_image.len()
    );
    ...
    assert!(
        ...
        "X coordinate not varying horizontally"
    );
    assert!(
    ...
    assert!(
        ...
        "Y coordinate not varying vertically"
    );
    assert!(
    ...
    assert!(
        (top_left.2 as i32 - expected_blue as i32).abs() < 10,
        "Z coordinate not at expected value"
    );
    assert!(
        (center.2 as i32 - expected_blue as i32).abs() < 10,
        "Z coordinate not at expected value"
    );
...
fn test_debug2_shader_voxel_bounds() {
    ...
    */
    ...
    render_loop
        .set_shader("slice_debug2")
        .expect("Failed to switch to debug2 shader");

    // Render with debug2 shader
    let debug_image = render_loop.render_to_buffer().expect("Failed to render");

    println!(
        "Debug2 shader render complete. Image size: {} bytes",
        debug_image.len()
    );
    ...
    assert!(
        (center_color.0 as i32 - expected).abs() < 20,
        "Unexpected X voxel coordinate"
    );
    assert!(
        (center_color.1 as i32 - expected).abs() < 20,
        "Unexpected Y voxel coordinate"
    );
    assert!(
        (center_color.2 as i32 - expected).abs() < 20,
        "Unexpected Z voxel coordinate"
    );
}
