...
fn create_single_voxel_volume(voxel_pos: [usize; 3]) -> DenseVolume3<f32> {
    ...
    println!("Created volume with bright voxel at {:?}, index {}, value {}", voxel_pos, idx, data[idx]);
...
async fn test_view_synchronization_at_crosshair() {
    ...
    let (texture_idx, world_to_voxel) = service.upload_volume_3d(&volume)
        .expect("Failed to upload volume");
    
    println!("Uploaded volume to texture_idx {}, world_to_voxel: {:?}", texture_idx, world_to_voxel);
...
async fn test_synchronized_axial(service: &mut RenderLoopService, texture_idx: u32, size: u32, crosshair: [f32; 3]) {
...
async fn test_synchronized_coronal(service: &mut RenderLoopService, texture_idx: u32, size: u32, crosshair: [f32; 3]) {
...
async fn test_synchronized_sagittal(service: &mut RenderLoopService, texture_idx: u32, size: u32, crosshair: [f32; 3]) {
...
async fn test_crosshair_movement_updates_views() {
    ...
    let (texture_idx, _world_to_voxel) = service.upload_volume_3d(&volume)
        .expect("Failed to upload volume");
