...
fn verify_checkerboard_pattern(buffer: &[u8], size: u32, view_name: &str) {
    ...
    println!(
        "  {} view: found {} intensity transitions",
        view_name, transitions
    );
...
fn create_test_volume() -> volmath::DenseVolume3<f32> {
    ...
    let max_dist = ((dims[0] as f32).powi(2) + (dims[1] as f32).powi(2) + (dims[2] as f32).powi(2))
        .sqrt()
        / 2.0;
