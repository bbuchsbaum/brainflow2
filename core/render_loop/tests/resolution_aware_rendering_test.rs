...
mod tests {
    ...
    fn test_multi_resolution_world_alignment() {
        pollster::block_on(async {
            ...
            let detail_voxel =
                detail_transform.transform_point(&nalgebra::Point3::from(world_origin));
            assert_abs_diff_eq!(
                detail_voxel.coords,
                Vector3::new(64.0, 64.0, 32.0),
                epsilon = 0.001
            );
        });
    }
    ...
    fn test_world_space_multi_texture_upload() {
        pollster::block_on(async {
            ...
            let volumes = test_fixtures::TestVolumeSet::create_aligned();
            let (anat_idx, anat_transform) = service
                .upload_volume_3d(&volumes.anatomical)
                .expect("Failed to upload anatomical");
            let (func_idx, func_transform) = service
                .upload_volume_3d(&volumes.functional)
    ...
    fn test_smart_format_selection() {
        pollster::block_on(async {
            ...
            service
                .enable_smart_texture_management(512)
            ...
            let (_anat_idx, _) = service
                .upload_volume_smart(&volumes.anatomical, None)
                .expect("Failed to upload anatomical with smart manager");
            let (func_idx, _) = service
                .upload_volume_smart(&volumes.functional, None)
                .expect("Failed to upload functional with smart manager");
            let (_detail_idx, _) = service
                .upload_volume_smart(&volumes.detail_patch, None)
            ...
            service
                .release_smart_texture(func_idx)
    ...
    fn test_resolution_boundary_sampling() {
        pollster::block_on(async {
            ...
            for pos in boundary_positions {
                ...
                assert!(
                    voxel_pos.x <= 0.5
                        || voxel_pos.x >= dims[0] - 0.5
                        || voxel_pos.y <= 0.5
                        || voxel_pos.y >= dims[1] - 0.5
                        || voxel_pos.z <= 0.5
                        || voxel_pos.z >= dims[2] - 0.5,
                    "Position {:?} -> voxel {:?} not at boundary",
                    pos,
                    voxel_pos
                );
            }
        ...
        pollster::block_on(async {
            ...
            for i in 0..render_loop::multi_texture_manager::MAX_TEXTURES {
                ...
                match service.upload_volume_3d(&volume) {
                    ...
                    Err(_) => {
                        panic!(
                            "Failed to upload volume {} of {}",
                            i,
                            render_loop::multi_texture_manager::MAX_TEXTURES
                        );
                    }
                }
            }
