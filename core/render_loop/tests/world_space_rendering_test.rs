...
fn test_world_space_rendering_workflow() {
    pollster::block_on(async {
        ...
        if let Some(layer_storage) = service.layer_storage_manager.as_mut() {
            ...
            println!("Successfully configured {} layers for world-space rendering", layer_storage.active_count());
        }
        
        // Verify multi-texture bind group is created
        assert!(service.multi_texture_manager.as_ref().unwrap().bind_group().is_some());
        println!("Multi-texture bind group created successfully");
        
        // Verify layer storage bind group is created
        assert!(service.layer_storage_manager.as_ref().unwrap().bind_group().is_some());
...
fn test_texture_format_compatibility() {
    pollster::block_on(async {
        ...
        // R8Unorm should work
        let result = service.upload_volume_multi_texture(
            &test_volume,
            wgpu::TextureFormat::R8Unorm,
        );
        assert!(result.is_ok());
