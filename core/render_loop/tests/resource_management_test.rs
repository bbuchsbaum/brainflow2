...
async fn test_volume_upload_release_cycle() {
    ...
    for i in 0..10 {
        ...
        println!("Iteration {}: Uploaded volume with texture_idx {}", i, texture_idx);
        ...
        service.release_volume(texture_idx).expect("Failed to release volume");
    }
    ...
    assert!(unique_indices.len() < 10, "Texture indices should be reused, but found {} unique indices", unique_indices.len());
...
async fn test_multiple_simultaneous_volumes() {
    ...
    for (i, (texture_idx, _)) in volumes.iter().enumerate() {
        ...
        service.add_render_layer(*texture_idx, opacity, (0.0, 0.0, 1.0, 1.0))
    ...
    println!("Composite render produced {} unique intensity values", unique_intensities);
    assert!(unique_intensities > 2, "Should see multiple intensity levels from volume composite");
...
async fn test_large_volume_handling() {
    ...
    for size in sizes {
        ...
        println!("  Volume contains {} voxels (~{} MB)", 
                 voxel_count, memory_estimate / (1024 * 1024));
        ...
        match service.upload_volume_3d(&volume) {
            Ok((texture_idx, _)) => {
                ...
                let non_zero = buffer.chunks(4).filter(|rgba| rgba[0] > 0).count();
                println!("  ✓ Rendered successfully with {} non-zero pixels", non_zero);
            }
            Err(e) => {
                ...
                if size > 256 {
                    ...
                    panic!("Should be able to handle {}x{}x{} volumes", size, size, size);
                }
            }
        }
    }
