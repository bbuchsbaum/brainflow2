...
async fn test_threshold_based_layers(service: &mut RenderLoopService, size: u32) {
    ...
    service
        .update_layer_threshold(0, 0.7, 1.0)
    ...
    service
        ...
        .expect("Failed to update layer intensity");
    service
        .update_layer_threshold(0, 0.4, 1.0)
        .expect("Failed to update layer threshold");
...
async fn test_multi_threshold_blend(service: &mut RenderLoopService, size: u32) {
    ...
    service
        .add_render_layer(0, 0.5, (0.0, 0.0, 1.0, 1.0))
    ...
    service
        .update_layer_threshold(0, 0.1, 0.3)
...
fn verify_threshold_render(
    ...
    let avg_value = if pixel_count > 0 {
        value_sum / pixel_count as f32
    } else {
        0.0
    };

    println!(
        "  {} - Pixels in region: {}, Average value: {:.3}",
...
fn verify_multi_threshold(buffer: &[u8], size: u32) {
    ...
    println!(
        "  Multi-threshold - Inner: {:.3}, Outer: {:.3}",
...
async fn test_multi_volume_different_resolutions() {
    ...
    // Upload both volumes
    let (tex1, transform1) = service
        .upload_volume_3d(&vol1)
        .expect("Failed to upload volume 1");
    let (tex2, transform2) = service
        .upload_volume_3d(&vol2)
    ...
    service
        .add_render_layer(tex1, 0.5, (0.0, 0.0, 1.0, 1.0))
        .expect("Failed to add volume 1 layer");
    service
        .add_render_layer(tex2, 0.5, (0.0, 0.0, 1.0, 1.0))
    ...
    assert!(
        non_zero_count > 0,
        "Multi-volume overlay should produce visible pixels"
    );
...
fn verify_multi_volume_patterns(buffer: &[u8], size: u32, test_name: &str) {
    ...
    println!(
        "{}: Found {} distinct intensity levels",
        test_name,
        intensity_counts.len()
    );
    assert!(
        intensity_counts.len() > 2,
        "Should see mixed intensities from both volumes"
    );
}
...
fn verify_sphere_and_box(buffer: &[u8], size: u32) {
    ...
    if box_only_x < size && box_only_y < size {
        ...
        println!(
            "  Center pixel (128,128) -> world [40,40]: value = {}",
            center_value
        );
        println!(
            "  Box-only pixel (200,128) -> world [~62.5,40]: value = {}",
            box_only_value
        );
        ...
        if outside_value > background_threshold && outside_value < 95 {
            println!(
                "  Note: Outside value {} is likely the sRGB clear color",
                outside_value
            );
        }
