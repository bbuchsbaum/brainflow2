...
use render_loop::{
    ...
    OptimizedRenderer, PerformanceMonitor, RenderLoopError,
};
use volmath::{DenseVolume3, traits::Volume, space::GridSpace};
...
impl BenchmarkResults {
    fn print_summary(&self) {
        ...
        println!("Total voxels: {:.2}M", self.total_voxels as f32 / 1_000_000.0);
...
async fn benchmark_shader_performance(
    ...
    for (i, vol) in volumes.iter().enumerate() {
        let (handle, transform) = service.upload_volume_3d(vol)
    ...
    // Create layer infos
    let layers: Vec<LayerInfo> = volumes.iter().enumerate()
        .zip(layer_configs.iter())
        .map(|((i, _vol), (opacity, blend_mode, colormap_id))| {
            LayerInfo {
    ...
    let total_voxels: usize = volumes.iter()
        .map(|v| {
    ...
    for i in 0..frame_count {
        ...
        // Render frame
        let mut encoder = service.device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
    ...
    optimized_renderer.update_layers(&service.device, &service.queue, &layers, &dimensions, &transforms);
    ...
    optimized_renderer.load_optimized_shader(&service.device)
    ...
    let improvement = ((original_results.avg_frame_time_ms - optimized_results.avg_frame_time_ms) 
        / original_results.avg_frame_time_ms) * 100.0;
...
fn test_single_layer_performance() {
    pollster::block_on(async {
        ...
        let mut optimized_renderer = OptimizedRenderer::new(&service.device)
            .expect("Failed to create OptimizedRenderer");
        ...
        assert!(optimized.avg_frame_time_ms < original.avg_frame_time_ms,
...
fn test_multi_layer_blending_performance() {
    pollster::block_on(async {
        ...
        let improvement = ((original.avg_frame_time_ms - optimized.avg_frame_time_ms) 
            / original.avg_frame_time_ms) * 100.0;
...
fn test_transparent_layer_early_exit() {
    pollster::block_on(async {
        ...
        let test_volumes: Vec<_> = layer_configs.iter()
            .enumerate()
...
fn test_memory_bandwidth_optimization() {
    pollster::block_on(async {
                ...
                // Smooth gradient (good cache coherence)
                {
                    ...
                    for z in 0..dims[2] {
                        for y in 0..dims[1] {
                            for x in 0..dims[0] {
                                let idx = z * dims[0] * dims[1] + y * dims[0] + x;
                                data[idx] = (x as f32 / 256.0 + y as f32 / 256.0 + z as f32 / 256.0) / 3.0;
                            }
                        }
                    }
        ...
        assert!(smooth_improvement > 5.0, "Should see improvement for smooth data");
        assert!(check_improvement > 0.0, "Should see some improvement for checkerboard");
    });
}
...
fn test_performance_summary() {
    ...
    pollster::block_on(async {
        ...
        let scenarios = vec![
            ("Single High-Res Layer", vec![0], vec![(1.0, BlendMode::Normal, 0)]),
            ("Three Layer Blend", vec![0, 1, 2], vec![
                (1.0, BlendMode::Normal, 0),
                (0.5, BlendMode::Additive, 1),
                (0.3, BlendMode::Multiply, 2),
            ]),
            ("Many Transparent Layers", vec![0, 1, 2, 0, 1], vec![
                (1.0, BlendMode::Normal, 0),
        ...
        for (name, volume_indices, layer_configs) in scenarios {
            ...
            let test_volumes: Vec<_> = volume_indices.iter()
            ...
            println!("  Original: {:.2}ms ({:.1} FPS)", 
                    original.avg_frame_time_ms, original.fps);
            println!("  Optimized: {:.2}ms ({:.1} FPS)", 
