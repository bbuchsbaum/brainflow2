...
fn test_upload_and_render_volume() {
    ...
    for &slice_idx in &slice_indices {
        match service.upload_slice(&volume, 2, slice_idx) {
            // Z-axis slice
            Ok((atlas_idx, u_min, v_min, u_max, v_max)) => {
                ...
                let layer_idx = service
                    .add_render_layer(atlas_idx, 1.0, (u_min, v_min, u_max, v_max))
    ...
    if let Some(&first_layer) = layer_indices.first() {
        service
            .update_layer(first_layer, 0.5, 2) // 50% opacity, colormap 2
...
fn test_volume_metadata_tracking() {
    ...
    // Upload a slice
    let (atlas_idx, _, _, _, _) = service
        .upload_slice(&volume, 2, 10)
...
fn test_atlas_layer_allocation() {
    ...
    println!(
        "Successfully allocated {} unique atlas layers",
        allocated_layers.len()
    );
}
