...
fn test_voxel_to_world_inverse() {
    ...
    let anat_v2w = anat_tfm
        .try_inverse()
        .expect("Anatomical transform not invertible");
    let func_v2w = func_tfm
        .try_inverse()
        .expect("Functional transform not invertible");
    let detail_v2w = detail_tfm
        .try_inverse()
        .expect("Detail transform not invertible");
    ...
    assert!(
        (anat_world - test_point).norm() < 1e-6,
        "Anatomical round-trip failed"
    );
    ...
    assert!(
        (func_world - test_point).norm() < 1e-6,
        "Functional round-trip failed"
    );
    ...
    assert!(
        (detail_world - test_point).norm() < 1e-6,
...
fn test_transform_resolution() {
    ...
    assert_eq!(
        (anat_x - anat_origin).x,
        1.0,
        "Anatomical X resolution incorrect"
    );
    assert_eq!(
        (anat_y - anat_origin).y,
        1.0,
        "Anatomical Y resolution incorrect"
    );
    assert_eq!(
        (anat_z - anat_origin).z,
        1.0,
        "Anatomical Z resolution incorrect"
    );
    ...
    assert_eq!(
        (func_x - func_origin).x,
        0.5,
        "Functional X resolution incorrect"
    );
    assert_eq!(
        (func_y - func_origin).y,
        0.5,
        "Functional Y resolution incorrect"
    );
    assert_eq!(
        (func_z - func_origin).z,
        0.25,
        "Functional Z resolution incorrect"
    );
    ...
    assert_eq!(
        (detail_x - detail_origin).x,
        2.0,
        "Detail X resolution incorrect"
    );
    assert_eq!(
        (detail_y - detail_origin).y,
        2.0,
        "Detail Y resolution incorrect"
    );
    assert_eq!(
        (detail_z - detail_origin).z,
        2.0,
        "Detail Z resolution incorrect"
    );
}
...
fn test_negative_voxel_handling() {
    ...
    for (world_point, desc) in boundary_tests {
        ...
        assert!(
            voxel.x >= 0.0 && voxel.x <= 256.0,
            "{}: X out of bounds",
            desc
        );
        assert!(
            voxel.y >= 0.0 && voxel.y <= 256.0,
            "{}: Y out of bounds",
            desc
        );
        assert!(
            voxel.z >= 0.0 && voxel.z <= 256.0,
            "{}: Z out of bounds",
            desc
        );
    }
