...
fn test_multi_resolution_overlay() {
    pollster::block_on(async {
        ...
        let (anat_id, _anat_transform) = service
            .upload_volume_3d(&volumes.anatomical)
            .expect("Failed to upload anatomical");

        let (func_id, _func_transform) = service
...
fn test_partial_fov_rendering() {
    pollster::block_on(async {
        ...
        // Upload both volumes
        let (full_id, _full_transform) = service
            .upload_volume_3d(&full_brain)
            .expect("Failed to upload full FOV");

        let (partial_id, _partial_transform) = service
            .upload_volume_3d(&partial_fov)
...
fn test_binary_mask_rendering() {
    pollster::block_on(async {
        ...
        // Upload mask
        let (mask_id, _mask_transform) = service
...
fn assert_shows_both_layers(image_data: &[u8], width: u32, height: u32) {
    ...
    assert!(
        has_variation,
        "Image has no variation - likely only showing one layer"
    );
}
