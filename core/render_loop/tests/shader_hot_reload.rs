...
fn test_shader_hot_reload() {
    ...
    // Check for updates
    let updated = service.check_shader_updates()
    ...
    fs::write(shader_path, &original_content)
        .expect("Failed to restore shader");
...
fn test_pipeline_recreation_on_slice_shader_change() {
    ...
    service._create_render_pipeline()
    ...
    let original_content = fs::read_to_string(shader_path)
        .expect("Failed to read slice shader");
    ...
    assert!(updated, "Slice shader change should trigger pipeline update");
    
    // Verify pipeline can still be retrieved (it will be recreated on demand)
    let _pipeline = service.get_pipeline("slice")
