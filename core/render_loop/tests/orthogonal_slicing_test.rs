...
async fn test_axial_view(service: &mut RenderLoopService, size: u32, crosshair: [f32; 3]) {
    ...
    use render_loop::render_state::{RenderPassConfig, RenderPassType};
    service.configure_render_pass(
        RenderPassType::Main,
        RenderPassConfig {
...
fn verify_axial_pattern(buffer: &[u8], size: u32) {
    ...
    assert!(
        non_zero_rgb_pixels > 0,
        "Axial view should have some visible pixels"
    );
...
fn verify_coronal_pattern(buffer: &[u8], size: u32) {
    ...
    assert!(
        max_val - min_val > 0.2,
        "Coronal view should show horizontal stripes"
    );
}
...
fn verify_sagittal_pattern(buffer: &[u8], size: u32) {
    ...
    assert!(
        ...
        "Sagittal view should show Y stripes or Z gradient (diff: {})",
        max_val - min_val
    );
}
