...
async fn test_3d_texture_upload() -> Result<(), RenderLoopError> {
    ...
    for i in 0..4 {
        for j in 0..4 {
            ...
            assert!(
                (world_to_voxel[(i, j)] - expected).abs() < 1e-6,
                "World-to-voxel transform incorrect at ({}, {})",
                i,
                j
            );
        }
...
async fn test_3d_texture_with_rotation() -> Result<(), RenderLoopError> {
    ...
    let space =
        <volmath::NeuroSpace as NeuroSpaceExt>::from_affine_matrix4(dims.to_vec(), affine.clone())
    ...
    for i in 0..4 {
        for j in 0..4 {
            assert!(
                (world_to_voxel[(i, j)] - expected_w2v[(i, j)]).abs() < 1e-6,
