    ...
    pub fn render_volume_slice(
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                let voxel_coord_h = world_to_voxel
                    * nalgebra::Vector4::new(world_point.x, world_point.y, world_point.z, 1.0);
                ...
                if voxel_coord.x < 0.0
                    || voxel_coord.x >= dims[0] as f32 - 1.0
                    || voxel_coord.y < 0.0
                    || voxel_coord.y >= dims[1] as f32 - 1.0
                    || voxel_coord.z < 0.0
                    || voxel_coord.z >= dims[2] as f32 - 1.0
                {
                    continue;
                }
