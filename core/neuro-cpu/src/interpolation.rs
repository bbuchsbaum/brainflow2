...
pub fn sample_trilinear(volume: &dyn Volume, voxel: [f32; 3]) -> Option<f32> {
    ...
    if x0 < 0 || x0 >= dims[0] as i32 ||
       y0 < 0 || y0 >= dims[1] as i32 ||
       z0 < 0 || z0 >= dims[2] as i32 {
        return None;
    }
...
pub fn sample_cubic(volume: &dyn Volume, voxel: [f32; 3]) -> Option<f32> {
    ...
    panic!("Cubic interpolation not yet implemented - use sample_trilinear or sample_nearest instead");
}
