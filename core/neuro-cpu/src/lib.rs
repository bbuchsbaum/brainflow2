...
use std::sync::Arc;
use neuro_types::{
    SliceProvider, CompositeRequest, RgbaImage, 
    Result as NeuroResult,
    VolumeHandle as NeuroVolumeHandle,
};
use neuro_core::{
    VolumeStore, Volume,
    VolumeHandle as CoreVolumeHandle,
};
...
impl CpuSlicer {
    ...
    fn sample_volume_at_world(
        ...
        let world_to_voxel_f64 = nalgebra::Matrix4::<f64>::new(
            world_to_voxel[(0, 0)] as f64, world_to_voxel[(0, 1)] as f64, world_to_voxel[(0, 2)] as f64, world_to_voxel[(0, 3)] as f64,
            world_to_voxel[(1, 0)] as f64, world_to_voxel[(1, 1)] as f64, world_to_voxel[(1, 2)] as f64, world_to_voxel[(1, 3)] as f64,
            world_to_voxel[(2, 0)] as f64, world_to_voxel[(2, 1)] as f64, world_to_voxel[(2, 2)] as f64, world_to_voxel[(2, 3)] as f64,
            world_to_voxel[(3, 0)] as f64, world_to_voxel[(3, 1)] as f64, world_to_voxel[(3, 2)] as f64, world_to_voxel[(3, 3)] as f64,
        );
        
        let voxel_vec_f64 = world_to_voxel_f64 * world_vec;
        
        // Check for NaN/Inf in f64 space
        if !voxel_vec_f64.x.is_finite() || !voxel_vec_f64.y.is_finite() || !voxel_vec_f64.z.is_finite() {
            return None;
        }
        
        // Convert back to f32 for sampling (but maintain precision up to this point)
        let mut voxel = [voxel_vec_f64.x as f32, voxel_vec_f64.y as f32, voxel_vec_f64.z as f32];
...
impl SliceProvider for CpuSlicer {
    fn composite_rgba(&self, request: &CompositeRequest) -> NeuroResult<RgbaImage> {
        ...
        for layer in layers {
            let inv = layer.world_from_voxel.try_inverse()
                .ok_or_else(|| neuro_types::Error::TransformError(
                    format!("Cannot invert transform for volume {:?}", layer.volume_id)
                ))?;
