...
- `population_slice/export.rs` implements `export_population_summary`: freeze source/mask hashes from a completed view, validate the same participant partition, and reuse `sample_row`/`reduce_plane` over full native-grid blocks. One export admission permit is retained by active blocking workers. Output vectors are capped at 128 MiB and row/reduction blocks budget 32 MiB independently of decoded-cache, mask and writer scratch. Write summary/coverage NIfTIs and calculation JSON into an owned staging directory; recheck all source stamps and cancellation before publishing a unique bundle. Preserve affine and explicit stack index; excluded coverage is NaN. The JSON records hashes and executable operands, not embedded source images or a complete application session/model.

- `population_slice/replay.rs` implements `replay_population_summary` for exported v1 records. It admits one replay at a time, bounds record input to 4 MiB and each fixed-name artifact to 256 MiB, checks identities/digests and bundle stamps, then calls `export_checked`. Recomputed grid/source revisions and exact summary/coverage artifact bytes must agree before publication. Owned staging is removed on mismatch/cancellation; the new record links its parent path and SHA. Record context is descriptive and never drives selection/weighting. This is strict bundle recalculation, not workspace restore or immutable source retention.

- `open_population_summary` shares the strict bounded bundle reader with replay, then verifies every frozen source/frame/grid and mask through a private one-pixel plane. It returns executable operands and descriptive context after source/bundle guards pass; no output files or live plane-cache replacement. Opening verifies input/artifact integrity, not full-grid result equivalence. `replay_population_summary` remains the exact recalculation gate.
