...
const isComponentItem = (item: ContentItem): item is ComponentItem =>
  item.type === "component";
...
class LayoutServiceImpl implements LayoutService {
  ...
  private addWorkspaceTabsFromStore(): void {
    ...
    if (!centerStack) {
      console.warn(
        "[LayoutService] Cannot rehydrate workspaces - center stack unavailable",
      );
      return;
    }
    ...
    if (activeItem && isComponentItem(activeItem)) {
      try {
        ...
        console.warn(
          "[LayoutService] Failed to activate workspace after layout restore:",
          error,
        );
      }
    }
  ...
  private loadLayoutAndRehydrate(config: LayoutConfig): boolean {
    if (!this.layoutRef || typeof this.layoutRef.loadLayout !== "function") {
      console.warn(
        "[LayoutService] Cannot load layout - layout not initialized",
      );
      return false;
    }
      ...
      requestAnimationFrame(() => {
        try {
          ...
          try {
            ...
            console.error(
              "[LayoutService] Failed to add component:",
              fallbackError,
            );
          }
        }
  ...
  ensureSurfaceView(surfaceHandle: string, path?: string): void {
    ...
    if (!centerStack) {
      console.warn(
        "[LayoutService] Cannot ensure surface view - center stack unavailable",
      );
      return;
    }
    ...
    if (existing && isComponentItem(existing)) {
      try {
        ...
        console.warn(
          "[LayoutService] Failed to activate existing surface view tab:",
          error,
        );
      }
      return;
    ...
    const title =
      path?.split("/").pop() || `Surface ${surfaceHandle.slice(0, 8)}`;
    this.addComponent({
      type: "component",
  ...
  closeSurfaceViewTabs(surfaceHandle: string): void {
    ...
    if (!centerStack) {
      console.warn(
        "[LayoutService] Cannot close surface tabs - center stack unavailable",
      );
      return;
    }
  ...
  focusSidebarPanel(panelType: SidebarPanelType): void {
    ...
    if (!sidebarStack || !this.layoutRef) {
      console.warn(
        `[LayoutService] Cannot focus ${resolvedType} - sidebar stack unavailable`,
      );
      return;
    }
    ...
    if (existingPanel && isComponentItem(existingPanel)) {
      try {
        ...
        console.warn(
          `[LayoutService] Failed to focus existing ${resolvedType}:`,
          error,
        );
      }
      return;
    ...
    try {
      ...
      console.log(
        `[LayoutService] Added and focused missing sidebar panel: ${resolvedType}`,
      );
    } catch (error) {
      console.error(
        `[LayoutService] Failed to add/focus sidebar panel ${resolvedType}:`,
        error,
      );
    }
  }
  ...
  resetToDefaultLayout(): boolean {
    return this.loadLayoutAndRehydrate(
      cloneLayoutConfig(DEFAULT_LAYOUT_CONFIG),
    );
  }
}
