...
- Common population masks flow through `PopulationProbeActions`, image requests and both scalar/trace sample requests. Mask selection is guarded by set ID and import session revision. Require returned mask provenance; include its digest when validating saved response order. A changed mask invalidates support/results and releases prior image contexts, while a separate dataset/feature scale key preserves compatible color scales. Do not silently reuse an unmasked result or infer a binary threshold.

- `PopulationExportService` freezes completed display operands, exact source/mask digests and descriptive metadata before opening the directory chooser. Later selection or file-dialog delays cannot change the exported estimand. Native export uses cancellation tickets; the mounted export control cancels on teardown. Export is a separate saved bundle, not a replacement for live view or legacy compare materialization.

- `PopulationExportService.chooseAndReplay` chooses an exported record and output parent, then invokes replay with only those paths and a cancellation ticket. It does not read live selection as calculation operands or mutate Studio state. Both file-picker cancellations stop the operation. Replay errors remain visible and never retry against changed sources or substitute current population settings.

- `PopulationRestoreService` opens a native-verified record into the existing Studio bootstrap, guarded by destination workspace, set and import generation. Frozen inputs live on `set.savedPopulation` and flow through both population query builders; generic member source paths remain null to prevent legacy materialization from dropping hashes/frames. Executable calculation groups, not descriptive context, define restored weighting. Partial saved identity refuses unknown selected members; unavailable metadata/reference membership stays explicit. `restoredView` seeds local view/scale state once per import, and subsequent export records its opened parent.
