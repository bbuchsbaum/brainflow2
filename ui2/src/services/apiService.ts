...
export class ApiService {
  ...
  private buildSingleViewRenderPayload(
    ...
    if (sliceOverride && viewType) {
      const axisIndex =
        sliceOverride.axis === "x" ? 0 : sliceOverride.axis === "y" ? 1 : 2;
      const newWorldMm = [...viewState.crosshair.world_mm] as [
        number,
        number,
        number,
      ];
      ...
      apiDebugLog(
        `[ApiService] Original crosshair: [${viewState.crosshair.world_mm}]`,
      );
      apiDebugLog(`[ApiService] Modified crosshair: [${newWorldMm}]`);
    }

    const { visibleLayers, payloadLayers } =
      this.getVisibleRenderLayers(viewState);
  ...
  private async requestSingleViewBytes(
    ...
    performance.measure(
      `render:serialize`,
      `${markId}-serialize-start`,
      `${markId}-serialize-end`,
    );
    performance.clearMarks(`${markId}-serialize-start`);
    performance.clearMarks(`${markId}-serialize-end`);
    apiDebugLog(
      `[ApiService] render_view payload serialized to ${stateJson.length} bytes`,
    );
  ...
  async applyAndRenderViewStateCore(
    ...
    const startTime = performance.now();
    apiDebugLog(
      `[ApiService ${startTime.toFixed(0)}ms] applyAndRenderViewStateCore called`,
    );
    ...
    if (!viewState.layers || viewState.layers.length === 0) {
      console.warn(
        "[ApiService] No layers in ViewState - returning empty image",
      );
      const canvas = new OffscreenCanvas(width, height);
      const ctx = canvas.getContext("2d");
      if (ctx) {
    ...
    const { payload: declarativeViewState, visibleLayerCount } =
      this.buildSingleViewRenderPayload(
        viewState,
        viewType,
        width,
        height,
    ...
    if (visibleLayerCount === 0) {
      console.warn(
        `[ApiService] WARNING: No visible layers to render! Returning empty image`,
      );
      const canvas = new OffscreenCanvas(width, height);
      const ctx = canvas.getContext("2d");
      if (ctx) {
    ...
    apiDebugLog(
      `[ApiService ${performance.now() - startTime}ms] Sending to backend:`,
    );
    ...
    apiDebugLog(
      `  - Full ViewState:`,
      JSON.stringify(declarativeViewState, null, 2),
    );

    const validation = validateRenderViewPayload(declarativeViewState);
    if (!validation.ok) {
      console.error(
        "[ApiService] Invalid render_view payload detected:",
        validation.errors,
      );
      if (import.meta.env.DEV) {
        throw new Error(
          `render_view payload validation failed: ${validation.errors.join("; ")}`,
        );
      }
    ...
    try {
      ...
      apiDebugLog(
        `[ApiService] render_view completed (${format}, ${imageData.length} bytes)`,
      );
      ...
      console.error(
        `[ApiService] Error type: ${(error as any)?.constructor?.name}`,
      );
  ...
  private async decodeRenderFrameToBitmap(
    ...
    if (frame.format === "rgba") {
      ...
      if (renderFlags.debugBrighten) {
        ...
        for (let i = 0; i < frame.rgba.length; i += 4) {
          ...
          brightenedRgba[i + 1] = Math.min(
            255,
            frame.rgba[i + 1] * brightenFactor,
          );
          brightenedRgba[i + 2] = Math.min(
            255,
            frame.rgba[i + 2] * brightenFactor,
          );
  ...
  async loadFile(
    path: string,
  ): Promise<import("./volume/VolumeApiService").VolumeHandle> {
  ...
  async recalculateAllViews(
    ...
    return getVolumeApiService().recalculateAllViews(
      volumeId,
      dimensionsByView,
      crosshairMm,
    );
  ...
  async requestLayerGpuResources(
    ...
    return getLayerGpuService().requestLayerGpuResources(
      layerId,
      volumeId,
      metadataOnly,
    );
  ...
  async waitForLayerReady(
    ...
    return getLayerGpuService().waitForLayerReady(
      layerId,
      timeoutMs,
      pollIntervalMs,
    );
  ...
  async batchRenderSlices(
    ...
    return getBatchRenderService().batchRenderSlices(
      viewStates,
      widthPerSlice,
      heightPerSlice,
    );
  ...
  async submitViewState(
    ...
    if (!validation.ok) {
      throw new Error(
        `submit_view payload validation failed: ${validation.errors.join("; ")}`,
      );
    }

    try {
      const diagnostics = await this.renderClient.submitView(
        declarativeViewState as any,
      );
  ...
  async renderViewStateMulti(
    ...
    if (visibleLayers.length === 0) {
      console.warn(
        "[ApiService] renderViewStateMulti called with no visible layers",
      );
    ...
    const payload: any = {
      ...
      requestedViews: viewTypes.map((viewType) => {
        ...
        if (!view) {
          throw new Error(
            `Requested view '${viewType}' missing from view state`,
          );
        }
        const [w, h] = view.dim_px || [512, 512];
        return {
          type: viewType,
          origin_mm: [...view.origin_mm, 1.0] as [
            number,
            number,
            number,
            number,
    ...
    if (!validation.ok) {
      console.error(
        "[ApiService] Invalid multi-view render payload detected:",
        validation.errors,
      );
      throw new Error(
        `render_views payload validation failed: ${validation.errors.join("; ")}`,
      );
    }
  ...
  private createSliceViewState(
    ...
    const slicePosition =
      bounds.min[axisIndex] + (sliceIndex * range) / totalSlices;
  ...
  setRawRGBA(enable: boolean) {
    ...
    apiDebugLog(
      `[ApiService] Raw RGBA transfer ${enable ? "enabled" : "disabled"}`,
    );
  }
  ...
  setDebugBrighten(enable: boolean) {
    ...
    apiDebugLog(
      `[ApiService] Debug brightening ${enable ? "enabled" : "disabled"}`,
    );
  }
