...
export class LayerService {
      ...
      if (!renderProps) {
        ...
        layerStore.setLayerRender(newLayer.id, {
          ...
          intensity: metadata?.dataRange ? 
            [Math.round((metadata.dataRange.min + (metadata.dataRange.max - metadata.dataRange.min) * 0.2) * 10) / 10,
             Math.round((metadata.dataRange.min + (metadata.dataRange.max - metadata.dataRange.min) * 0.8) * 10) / 10] : 
            [0, 100],
          threshold: [0, 0],
          colormap: 'gray',
  ...
  toggleVisibility(id: string, visible: boolean): void {
    // SINGLE SOURCE OF TRUTH: Only update opacity, remove dual state
    // The 'visible' property will be derived from opacity > 0
    this.patchLayer(id, { opacity: visible ? 1.0 : 0.0 });
    
    // Emit event for UI responsiveness (opacity change will trigger derived visible)
