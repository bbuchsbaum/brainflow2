...
export function useSliceViewModel(
  ...
  const crosshairVisible = useViewStateStore(
    (s) => s.viewState.crosshair.visible,
  );
  ...
  const primaryLayer = React.useMemo(
  ...
  // Hover readout is enabled when *any* visible layer has "show value on hover"
  // on — not just the top-most. The per-layer toggle (VolumePanel) otherwise
  // silently no-ops when set on a layer that isn't the primary one.
  const displayOptions = useDisplayOptionsStore((s) => s.options);
  const hoverEnabled = React.useMemo(
    () =>
      layers.some(
        (l) =>
          l.visible && (displayOptions.get(l.id)?.showValueOnHover ?? false),
      ),
    [layers, displayOptions],
  );
  ...
  const crosshairSettings = React.useMemo(() => {
    ...
    if (overrides) {
      return {
        ...
        activeThickness:
          overrides.thickness ?? crosshairSettingsRoot.activeThickness,
  ...
  const adaptive = useAdaptiveResolution({
    width: dims.width,
    height: canvasHeight,
  });
  ...
  const renderContext: RenderContext = React.useMemo(
    () => ({
      ...
      type: "slice",
      dimensions: {
        width: adaptive.renderWidth,
        height: adaptive.renderHeight,
      },
