...
export function useServicesInit() {
  useEffect(() => {
    // Start the playback loop OUTSIDE the one-time init guard. start() is
    // idempotent, so every effect instance (including a StrictMode remount
    // that skips the heavy init below) owns a matching start/stop pair and the
    // subscription never ends up torn down with nothing to restart it.
    getPlaybackService().start();
    const stopPlayback = () => {
    ...
    const initializeServices = async () => {
      try {
        ...
        // PlaybackService is started/stopped at the effect boundary below
        // (outside the one-time init guard) so its start/stop pair survives
        // a StrictMode double-invoke. Grab the handle here for window-exposure.
        ...
        serviceInitDebugLog(
          'Setting up coalescing middleware callback with optimized rendering...',
        );
        coalesceUtils.setBackendCallback(async (viewState, revisions) => {
          const callbackTime = performance.now();
          serviceInitDebugLog(
            `[useServicesInit ${callbackTime.toFixed(0)}ms] Backend callback invoked`,
          );
          serviceInitDebugLog(
            `[useServicesInit] Crosshair position:`,
            viewState.crosshair.world_mm,
          );
          serviceInitDebugLog(
            'Coalescing callback: ViewState update with',
            viewState.layers.length,
            'layers',
          );
        ...
        if (typeof window !== 'undefined') {
          ...
          (window as any).isRenderMultiViewEnabled = () =>
            useFeatureFlagStore.getState().multiViewBatch;
        }
        ...
        if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
          ...
          serviceInitDebugLog(
            '[useServicesInit] Services exposed to window.__BRAINFLOW_SERVICES for debugging',
          );
        }
        ...
        setTimeout(() => {
          ...
          serviceInitDebugLog(
            'TEST: Current viewState layers:',
            useViewStateStore.getState().viewState.layers.length,
          );
        }, 2000);
    ...
    // Tear down the playback loop on unmount. Other services use the global
    // `servicesInitialized` guard and intentionally persist for the app
    // lifetime; PlaybackService is the one subscription we own here, so stop it
    // to avoid a dangling event-bus listener / RAF loop.
    return stopPlayback;
  }, []);
}
