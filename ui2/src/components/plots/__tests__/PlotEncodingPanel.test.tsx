...
const richFrame: SampleFrame = {
  columns: [column('t', 'temporal'), column('value', 'quantitative'), column('group', 'nominal')],
...
describe('PlotEncodingPanel', () => {
  ...
  it('offers numeric AND categorical columns on the X channel for a continuous mark', () => {
    // A continuous mark (line/area/point) accepts a categorical x: it lays the
    // series on a point scale (e.g. the cross-set trace's member axis), so the
    // nominal `group` column is a valid X binding, not an excluded one.
    ...
    expect(xOpts).toContain('group');
  });

  it('offers a Color channel only when categorical columns exist', () => {
    const { unmount } = render(<PlotEncodingPanel modeId={M} frame={richFrame} />);
