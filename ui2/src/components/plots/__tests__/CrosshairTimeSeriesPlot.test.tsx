...
function setupFourDLayer(opts?: { dimensions?: [number, number, number]; numTimepoints?: number }) {
  ...
  layerStoreState.current = {
    getLayer: (id: string) =>
      id === 'vol-1'
        ...
        : undefined,
    getLayerMetadata: (id: string) =>
      id === 'vol-1' ? { worldToVoxel: 'matrix-stub', dimensions: [64, 64, 32] } : undefined,
  };
}
...
describe('crosshairTimeSeriesPlot.supports', () => {
  beforeEach(() => {
    ...
    voxelTransformMock.mockReturnValue([10, 20, 15] as [number, number, number]);
  });

  it('reports no-layer when no layer is selected', () => {
    setupFourDLayer();
    expect(crosshairTimeSeriesPlot.supports(ctxFor({ layerId: undefined }))).toMatchObject({
      supported: false,
      reason: 'no-layer',
    });
  });
  ...
  it('reports no-crosshair when 4D layer has no crosshair set', () => {
    setupFourDLayer();
    expect(crosshairTimeSeriesPlot.supports(ctxFor({ crosshairMm: undefined }))).toMatchObject({
      supported: false,
      reason: 'no-crosshair',
    });
  });
  ...
  it('reports supported for a 4D layer with crosshair', () => {
    setupFourDLayer();
    expect(crosshairTimeSeriesPlot.supports(ctxFor())).toEqual({ supported: true });
  });
...
describe('resolveTimeseriesVoxel', () => {
  ...
  it('returns rounded voxel coords plus volumeId for in-bounds 4D voxel', () => {
    ...
    voxelTransformMock.mockReturnValue([10.4, 20.6, 15.5] as [number, number, number]);
...
describe('CrosshairTimeSeriesPlotBody', () => {
  ...
  it('issues sample_voxel_timeseries with the sparkline-compatible request shape', async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith('sample_voxel_timeseries', {
      volumeId: 'volume-handle-1',
      voxelX: 10,
      voxelY: 20,
      voxelZ: 15,
  ...
  it('renders the SVG line plot once data arrives', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('crosshair-ts-svg')).toBeInTheDocument();
    });
  });
  ...
  it('throttles rapid crosshair changes to ≤30 Hz', async () => {
    ...
    voxelTransformMock.mockImplementation(() => {
      voxelCounter += 1;
      return [voxelCounter, voxelCounter, voxelCounter] as [number, number, number];
    });

    const { rerender } = render(<CrosshairTimeSeriesPlotBody ctx={ctxFor({ crosshairMm: [1, 1, 1] })} />);
  ...
  it('clears stale data when the crosshair leaves the volume', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('crosshair-ts-svg')).toBeInTheDocument());

    voxelTransformMock.mockReturnValue([100, 100, 100] as [number, number, number]);
    rerender(<CrosshairTimeSeriesPlotBody ctx={ctxFor({ crosshairMm: [50, 50, 50] })} />);

    await waitFor(() => {
      expect(screen.queryByTestId('crosshair-ts-svg')).toBeNull();
    });
  });
});
