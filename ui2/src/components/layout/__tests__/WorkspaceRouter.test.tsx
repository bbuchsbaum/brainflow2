...
vi.mock('@/components/studio/SetStudioWorkspace', () => ({
  SetStudioWorkspace: () => <div data-testid="route-set-studio">set-studio</div>,
}));
...
function makeWorkspace(id: string, type: WorkspaceType): Workspace {
  return {
    ...
    isActive: true,
    layoutConfig: {
      root: { type: 'component', componentType: 'EmptyView', title: id, componentState: {} },
    },
    panelStates: new Map(),
  };
}
...
describe('WorkspaceRouter', () => {
  it('renders a "not found" notice when the workspace id is not in the store', () => {
    ...
    expect(screen.getByTestId('workspace-router-not-found')).toBeInTheDocument();
  ...
  it('routes orthogonal-locked → OrthogonalViewContainer', () => {
    ...
    render(<WorkspaceRouter workspaceId="w-1" workspaceType="orthogonal-locked" />);
  ...
  it('routes orthogonal-flexible → OrthogonalPanelsWorkspace', () => {
    ...
    render(<WorkspaceRouter workspaceId="w-2" workspaceType="orthogonal-flexible" />);
  ...
  it('shows the disabled-flag notice for integrated when integratedWorkspaceV1 is off', () => {
    ...
    expect(screen.getByTestId('integrated-workspace-flag-disabled')).toBeInTheDocument();
  ...
  it('mounts the integrated shell when integratedWorkspaceV1 is on', () => {
    ...
    expect(screen.queryByTestId('integrated-workspace-flag-disabled')).toBeNull();
  });
});
