...
const VolumeLayerPanelContent: React.FC = () => {
  ...
  const [activeSidebarTab, setActiveSidebarTab] =
    useState<SidebarTabId>(SIDEBAR_TAB_DEFAULT);
  ...
  const metadataSelector = useMemo(
    () =>
      selectedLayerId
        ? (state: any) =>
            layerSelectors.getLayerMetadata(state, selectedLayerId)
        : () => undefined,
    [selectedLayerId],
  );
  ...
  const { isInitialized: serviceInitialized, error: initializationError } =
    useLayerPanelServices();
  // vol2surf M4: "View on surface" projection (enabled once a surface is loaded).
  const { projectVolume, surfaceIds } = useVolToSurfProjection();
  const canViewOnSurface = surfaceIds.length > 0;
    ...
    () =>
      viewStateLayer
        ? {
            ...
            interpolation: (viewStateLayer.interpolation || "linear") as
              | "nearest"
              | "linear",
  ...
  const handleReorder = useCallback(
    (newLayers: Layer[]) => {
      ...
      if (!serviceInitialized) {
        console.warn(
          "[VolumeLayerPanel] Ignoring reorder before LayerService initialization",
        );
        return;
      }
  ...
  const handleRemoveLayer = useCallback(
    (layerId: string) => {
      if (!serviceInitialized) {
        console.warn(
          "[VolumeLayerPanel] Ignoring remove before LayerService initialization",
        );
        return;
      }
      ...
      void getConfirmationService()
        ...
        .then((confirmed) => {
          ...
          void getLayerService()
            ...
            .catch((error) => {
              console.error(
                "[VolumeLayerPanel] Failed to remove layer:",
                error,
              );
            });
        });
  ...
  // vol2surf M4: project this volume layer onto a loaded surface and switch the
  // active view tab to the Integrated (surface + volume) workspace.
  const handleViewOnSurface = useCallback(
    (layerId: string) => {
      const layer = layers.find((item) => item.id === layerId);
      if (!layer) {
        return;
      ...
      if (surfaceIds.length === 0) {
        getEventBus().emit("ui.notification", {
          type: "warning",
          message: "Load a surface first to view this layer on a surface.",
        });
        return;
      }

      // Reconfigure the active view tab into Integrated mode (or create one).
      const ws = useWorkspaceStore.getState();
      if (ws.activeWorkspaceId) {
        ws.setActiveWorkspaceMode("integrated");
      } else {
        void ws.createWorkspace("integrated");
      }

      const volumeId = layer.volumeId || layer.id;
      void projectVolume(volumeId, surfaceIds[0], `${layer.name} on surface`, {
        colormap: "viridis",
        opacity: 1.0,
      }).then((result) => {
        if (!result) {
          getEventBus().emit("ui.notification", {
            type: "error",
            message: `Could not project "${layer.name}" onto the surface.`,
          });
        }
      });
    },
    [layers, surfaceIds, projectVolume],
  );
  ...
  const handleRenderUpdate = useCallback(
    (updates: Partial<LayerRender>) => {
      ...
      useViewStateStore.getState().setViewState((state) => {
        ...
        if (
          (sanitized as any).colormapId !== undefined &&
        ...
        if (
          ...
          !Object.is(layer.opacity, sanitized.opacity)
        ) {
        ...
        if (
          (sanitized as any).atlasPaletteKind &&
          (layer as any).atlasPaletteKind !==
            (sanitized as any).atlasPaletteKind
        ) {
        ...
        if (
          (sanitized as any).atlasPaletteSeed !== undefined &&
          (layer as any).atlasPaletteSeed !==
            (sanitized as any).atlasPaletteSeed
        ) {
          (layer as any).atlasPaletteSeed = (sanitized as any).atlasPaletteSeed;
          didChange = true;
        }
        if (
          (sanitized as any).atlasMaxLabel !== undefined &&
  ...
  return (
    <div
      className="flex flex-col h-full overflow-hidden bg-card text-card-foreground shadow-sm border border-border font-sans"
          ...
          >
            {SIDEBAR_TABS.map((tab, index) => {
              ...
              return (
                ...
                >
                  <span
                    className={`inline-flex items-center justify-center ${compactSidebarTabs ? "" : "gap-1.5"}`}
                  >
                    <TabIcon
                      className={`${compactSidebarTabs ? "h-4 w-4" : "h-3.5 w-3.5"} shrink-0`}
                      aria-hidden="true"
                    />
            ...
            >
              {layers.length > 0 ? (
                <LayerTable
                  ...
                  onViewOnSurface={handleViewOnSurface}
