...
const ROW_STYLE: React.CSSProperties = {
  display: 'grid',
  gridTemplateColumns: '52px 44px 64px 1fr',
  gap: '6px',
  alignItems: 'baseline',
  padding: '2px 8px',
...
const LogRow: React.FC<LogRowProps> = ({ entry }) => {
  return (
    <div
      ...
      style={ROW_STYLE}
    >
      <span style={{ color: 'var(--app-text-muted)' }}>{formatTimestamp(entry.timestamp)}</span>
      <span
        style={{
      ...
      <span
        style={{
          color: entry.level === 'error' ? 'var(--app-danger, #ef4444)' : 'var(--app-text-primary)',
          whiteSpace: 'pre-wrap',
  ...
  return (
    <div data-testid="log-panel" style={containerStyle}>
      <div style={TOOLBAR_STYLE}>
        <div role="group" aria-label="Filter log levels" style={{ display: 'inline-flex', gap: '2px' }}>
          {LEVELS.map((level) => {
              ...
              <button
                ...
                style={{
                  ...
                  borderColor: isActive
                    ...
                    : 'var(--app-border)',
                  color: isActive ? LEVEL_COLORS[level] : 'var(--app-text-muted)',
                  cursor: 'pointer',
                  fontSize: 'var(--app-role-section-size)',
                  letterSpacing: 'var(--app-role-section-tracking)',
                  textTransform: 'uppercase',
        ...
        <button
          ...
          style={{
            ...
            border: '1px solid var(--app-border)',
            color: autoScroll ? 'var(--app-text-primary)' : 'var(--app-text-muted)',
            padding: '0 6px',
            height: '20px',
            cursor: 'pointer',
            display: 'inline-flex',
        ...
        <button
          ...
          style={{
            ...
            color: entries.length === 0 ? 'var(--app-text-disabled)' : 'var(--app-text-muted)',
            display: 'inline-flex',
            alignItems: 'center',
            height: '20px',
          }}
