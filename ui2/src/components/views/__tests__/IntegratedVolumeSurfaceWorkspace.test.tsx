...
vi.mock("@/components/ui/ArrangementMenu", () => ({
  ArrangementMenu: ({ showSplit }: { showSplit?: boolean }) => (
    <div
      data-testid="arrangement-menu-stub"
      data-show-split={showSplit ? "true" : "false"}
    />
  ),
}));
...
describe("IntegratedVolumeSurfaceWorkspace", () => {
  ...
  it("composes the orthogonal and surface regions without a dock or a second inspector", () => {
    ...
    expect(
      screen.getByTestId("integrated-volume-surface-workspace"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("integrated-workspace-orthogonal-region"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("orthogonal-panels-workspace-stub"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("integrated-workspace-surface-region"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByTestId("bottom-workbench-dock"),
    ...
    expect(
      screen.queryByTestId("integrated-workspace-inspector-region"),
    ).not.toBeInTheDocument();
  });
  ...
  it("reflects the integrated split orientation from the store", () => {
    ...
    expect(
      screen.getByTestId("integrated-volume-surface-workspace"),
    ).toHaveAttribute("data-split", "horizontal");
    ...
    expect(
      screen.getByTestId("integrated-volume-surface-workspace"),
    ).toHaveAttribute("data-split", "vertical");
  });
