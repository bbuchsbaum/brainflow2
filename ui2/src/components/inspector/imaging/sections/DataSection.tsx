...
/**
 * Compact, read-only fact grid. Full charts (histogram, time-series) are
 * NOT shown here — they belong in the bottom Plot dock per Design.md §1.5.
 * The "Open in dock →" link hands off to the dock when applicable.
 */
...
function VolumeFacts({ item }: { item: SceneItem }) {
  ...
  if (dim) {
    const shape = ts ? `${dim[0]}×${dim[1]}×${dim[2]}×${ts.num_timepoints}` : `${dim[0]}×${dim[1]}×${dim[2]}`;
    facts.push({ label: 'Shape', value: shape });
  }
  if (metadata?.dataRange) {
    const { min, max } = metadata.dataRange;
    facts.push({ label: 'Range', value: `${formatNum(min)} — ${formatNum(max)}` });
  }
  if (metadata?.spacing) {
    const [sx, sy, sz] = metadata.spacing;
    facts.push({ label: 'Voxel size', value: `${formatNum(sx)}×${formatNum(sy)}×${formatNum(sz)} mm` });
  }
  if (ts?.tr != null) {
...
function SurfaceFacts({ item }: { item: SceneItem }) {
  ...
  if (surface.metadata.vertexCount) {
    facts.push({ label: 'Vertices', value: surface.metadata.vertexCount.toLocaleString() });
  }
  if (surface.metadata.faceCount) {
    facts.push({ label: 'Faces', value: surface.metadata.faceCount.toLocaleString() });
  }
...
function OpenInDockLink({ kind }: { kind: SceneItem['kind'] }) {
  const showsPlot =
    kind === 'volume-base' ||
    kind === 'volume-overlay' ||
