<script lang="ts">
	...
	async function initialize() {
		try {
			...
			const gpuInfo = await baseApi.request_layer_gpu_resources(layerSpec);
			console.log('GPU resources allocated:', gpuInfo);

			// Create offscreen render target
			const renderSize = 512;
			await baseApi.create_offscreen_render_target(renderSize, renderSize);

			// Configure render layer
			await baseApi.clear_render_layers();
			await baseApi.add_render_layer(
			...
			for (let i = 0; i < testCoordinates.length; i++) {
				...
				const images = {
					axial: await renderSlice(coord, 0, renderSize),
					coronal: await renderSlice(coord, 1, renderSize),
					sagittal: await renderSlice(coord, 2, renderSize)
				};
	...
	async function renderSlice(
		...
		await baseApi.set_crosshair(worldCoord);

		// Update frame for the specific plane
		// Using a reasonable field of view (e.g., 240mm)
		const viewSizeMm = 240;
		await baseApi.update_frame_for_synchronized_view(viewSizeMm, viewSizeMm, worldCoord, planeId);

		// Render frame
		await baseApi.render_frame();

		// Get image as base64
		const base64Image = await baseApi.render_to_image();
		return base64Image;
	}
