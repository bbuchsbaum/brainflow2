...
export const useVolumeStore = create<VolumeStoreState>()(
  subscribeWithSelector((set, get) => ({
    ...
    setLoading: (path, loading) => set((state) => {
    ...
    setError: (volumeId, error) => set((state) => {
      const errors = new Map(state.errors);
      if (error) {
...
export const volumeStoreSelectors = {
  ...
  // Get recent volumes
  recentVolumes: (limit = 5) => (state: VolumeStoreState) => {
    return Array.from(state.volumes.values())
