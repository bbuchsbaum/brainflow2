...
1.  **Fast Context:** The core Memory Bank files (`memory-bank/*.md`) provide a concise (< 5KB total) overview of the project's goals, architecture, tech stack, and current status. This allows the AI to load essential context quickly at the start of each session.
2.  **Single Source of Truth:** Detailed specifications, architectural decisions, plans, and guides reside **only** in the main `/docs` directory (`ADR-*.md`, `PLAN-*.md`, `GUIDE-*.md`, `SPEC-*.md`). The Memory Bank does *not* duplicate this content.
3.  **Linked References:** Memory Bank files contain explicit references (by filename) and brief summaries ("digests") of the key documents in `/docs`.
4.  **AI Guidance:** The AI assistant is instructed to *follow these references* and consult the detailed documents when specific information is needed for a task, rather than relying solely on the Memory Bank summary or asking the user to repeat information.
...
> *   **Docs:** **Follow references herein to detailed specs in `/docs` when needed.**

---
**Further Reading (Core Plans):**
*   `docs/PLAN-phase1-milestones.md` (Detailed Goals & DoD)
...
## 2. 🔗 Spec Digest (Key Documents in `/docs`)
...
*   **`PLAN-migration-phase1.md`:** Strategy for porting legacy TS code, scope of Rust ports (`volmath`, loaders), legacy TS integration plan. *(Last Sync: v1.1)*

*(Docs live in `/docs`; last digest sync [Current Date])*

---
**Further Reading:**
*   Consult the full documents listed above for implementation details.
*   `docs/repository_structure.md` (Detailed Folder Layout)
```
...
## 5. 🔗 Tech Spec References (Key Documents in `/docs` & `/schemas`)
...
*(Docs live in `/docs`, Schemas in `/schemas`; last digest sync [Current Date])*
...
*   **Immediate Next Steps:**
    1.  Execute tasks from `docs/DEV-bootstrap-guide.md`.
...
*   **Consult detailed docs in `/docs` when implementing features.**

---
**Further Reading:**
*   `docs/PLAN-phase1-milestones.md` (Current/Next Milestone Details)
*   `docs/Errata-phase1-v1.1.md` (Log of resolved spec inconsistencies)
```
...
| :-------- | :------------ | :------------------------------------ | :--------------------------------- |
| **M1**    | **Not Started** | Repo scaffold, API pkg, CI, UI Shell | `docs/PLAN-phase1-milestones.md#m1`|
| M2        | Not Started   | Rust Services, Bridge API, BIDS     | `docs/PLAN-phase1-milestones.md#m2`|
| M3        | Not Started   | NIfTI Load, WebGPU Slice Render     | `docs/PLAN-phase1-milestones.md#m3`|
| M4        | Not Started   | GIfTI Load, Surface View            | `docs/PLAN-phase1-milestones.md#m4`|
| M5        | Not Started   | Plotting Infra, Histogram Plot      | `docs/PLAN-phase1-milestones.md#m5`|
| M6        | Not Started   | Click->Plot, Matrix API             | `docs/PLAN-phase1-milestones.md#m6`|
| M7        | Not Started   | Atlas Load, Overlays, Legend        | `docs/PLAN-phase1-milestones.md#m7`|
| M8        | Not Started   | Polish, Integration, Builds         | `docs/PLAN-phase1-milestones.md#m8`|
...
---
**Further Reading:**
*   `docs/PLAN-phase1-milestones.md` (Detailed Milestone Plan)
*   `docs/Errata-phase1-v1.1.md` (Record of resolved spec inconsistencies)
```
...
```
*   *(Optional Script: `tools/update_doc_digest.sh` as provided previously can generate this table based on Git history and Markdown headers.)*
...
**Crucially, the Memory Bank files contain cross-references (as filenames, e.g., `docs/ADR-001-architecture.md`) to detailed specification documents located in the main `/docs` directory.**
...
REMEMBER: The Memory Bank is the starting point and index. The detailed specifications live in `/docs`. Follow the links!
```

## 5. Workflow Integration

1.  **Commit:** Add the `memory-bank/` directory with these populated files to Git.
2.  **Configure AI:** Set up Cline/Cursor to use the `.clinerules` file or paste the updated instructions into global Custom Instructions.
3.  **Start Session:** Begin a new chat/task by instructing the AI to "initialize memory bank" or "follow custom instructions".
4.  **Guide Interaction:** When asking for code or explanations requiring specific details, reference the relevant `/docs/*.md` file if the AI doesn't automatically pick it up from the Memory Bank context.
5.  **Maintain:** Regularly update `activeContext.md` and `progress.md`. After significant changes to specs/plans in `/docs`, update the corresponding brief summaries/links in the Memory Bank files (especially `systemPatterns.md` and `techContext.md`) and ask the AI to "update memory bank".
