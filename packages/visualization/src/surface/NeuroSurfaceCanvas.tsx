...
} from './surfaceReconciler.js';
import {
  DEFAULT_NEURO_SURFACE_DISPLAY_SETTINGS,
  DEFAULT_NEURO_SURFACE_LIGHTING_SETTINGS,
  DEFAULT_NEURO_SURFACE_MATERIAL_SETTINGS,
  DEFAULT_NEURO_SURFACE_PROJECTION_SETTINGS,
  type NeuroSurfaceCanvasProps,
...
interface NeuroSurfaceViewerInstance {
  ...
  requestRender: () => void;
  centerCamera: () => void;
  addSurface: (surface: unknown, handle: string) => void;
...
function hasRenderableGeometry(surface: SurfaceRenderable): boolean {
  return Boolean(
...
function removeRenderedSurface(
  ...
  try {
    const mesh = (rendered as any)?.mesh;
    if (mesh) {
...
const NeuroSurfaceCanvasInner: React.FC<NeuroSurfaceCanvasProps> = ({
  surfaces,
  width,
  height,
  viewpoint = 'lateral',
  ...
  const surfacesToRender = useMemo(() => {
    const unique = new Map<string, SurfaceRenderable>();
  ...
  useEffect(() => {
    ...
    const mountRenderedSurface = (item: SurfaceRenderable) => {
