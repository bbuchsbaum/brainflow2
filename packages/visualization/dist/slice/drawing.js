export function resolveSliceBorderColor() {
...
export function drawSliceBorder(ctx, placement, thickness, color = resolveSliceBorderColor()) {
    const safeThickness = Math.max(1, thickness);
    ctx.save();
    ctx.strokeStyle = color;
    ctx.lineWidth = safeThickness;
    const inset = safeThickness / 2;
    ctx.strokeRect(placement.x + inset, placement.y + inset, Math.max(0, placement.width - safeThickness), Math.max(0, placement.height - safeThickness));
    ctx.restore();
}
export function drawSliceViewerCrosshair({ ctx, canvasX, canvasY, bounds, style, }) {
    if (canvasX < bounds.x ||
